...
- **월간 운영 리포트**: 매월 **첫째 수요일** 08:30 KST에 **전월 리포트** 자동 발송 (2026-07-29 변경 — 매일 트리거 + `isFirstWednesdayOfMonth` + 월 중복 가드. 가드는 자동만 차단).
  - **구성 (2026-08-03 §8-7 개편 + 같은 날 렌더 리뷰 반영)**: ① Executive 요약(KPI **3카드** — 건수·인원 MoM + NPS 카드. 26년 누적 카드·요약 3줄 박스는 삭제) ② 사업부별 활용(확정 기준, **건수 있는 본부만**+기타 — 0건 본부 미표시) ③ 핵심 인사이트(큐레이션, 없으면 생략) ④ 인상 깊은 한마디(큐레이션, 출처 라벨) ⑤ 목적 도넛 ⑥ 관련 기사(**최대 5건** `REPORT_ARTICLE_LIMIT`, 썸네일 카드형 — 수동 링크 우선·미달분 자동 수집 보충) ⑦ **투자 대비 성과(ROI) 최하단·고정 수치**(구 명칭 'ROI 스냅샷' — 그래프 삭제, 연간 확정 가치 2.14억·총 투자 2.886억·BEP 1.31년·3년 +122.4%·5년 +270.7% + 실측 누적). 방문 이력·설문·성과 지표 섹션은 리포트에서 삭제(상세는 관리자 페이지가 소스). 블록별 try-catch 격리.
...
| 기사 검색 | **수동 큐레이션 우선 + 자동 보충 병합** (2026-08-03 전환): 관리자 큐레이션 UI(URL 입력 → 메타 태그로 제목·썸네일 자동 추출) 또는 시트 직접 입력한 링크가 먼저 실리고, 5건(`REPORT_ARTICLE_LIMIT`) 미달분만 Serper.dev(`SERPER_API_KEY`, 키워드 `MONTHLY_REPORT_QUERY`='LG전자 AI홈') → CSE 폴백(403 차단 중)으로 보충. 리포트 기사 섹션 설명문은 수동/혼합/자동 케이스별 자동 동기화 |
...
- [ ] **8/4(월) 팀장 리뷰**: 발송 확인 화면 → [나에게만 테스트 발송] → 수신본으로 리뷰.
- [ ] **7월분 전체 발송**: 리뷰 통과 시 **8/4 중** 확인 화면에서 「자동 발송 건너뛰기」 체크 + [전체 발송하기] 권장 (8/5에 하려면 자동 발송 08:30 **이전**이어야 중복 없음). 무수정 통과면 아무 조작 없이 8/5 08:30 자동 발송에 맡겨도 됨 — 피해야 할 것은 "체크 없이 수동 전체 발송"(중복 2통)뿐.
