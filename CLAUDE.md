...
- **지상주차 위치 약도**: 구글맵 스크린샷 대신 **인라인 SVG 약도**로 자체 제작 (디자인 시스템 톤 일치, 코드로 수정 용이). 위치 ①~④ + W6동 ThinQ Real 표기. ⚠️ PPT 원본이 저해상도라 위치 번호 배치는 추정 — 담당자 검증 후 좌표 조정 필요할 수 있음 (`<circle cx cy>` 좌표만 수정).
