...
- [ ] **방문자 서면 동의서 법무 확정**: `ThinQ_Real_Visit_Consent.html` v0.9(검토용 초안)를 법무팀 검토 후 v1.0 확정(초안 표기 제거). **3항(현장 대화 녹음 — 선택)을 실제 운영에 쓰는 시점에는 privacy.html v1.3 개정(수집 항목·목적·보유 추가) + FieldVoice `consent` 기록 방식과 정합 필요** — 3항 문안의 "요약 작성 완료 후 지체 없이 파기"는 FieldVoice 운영 기준과 대조해 확정할 것. 서명지(종이) 보관·파기 절차는 담당자 확인 필요 (시스템 미경유 — 보유 3년).
