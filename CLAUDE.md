...
- **보유 기간 "방문일로부터 3년"**: privacy.html §3 ↔ 폼 동의 문구 ↔ 서면 동의서(ThinQ_Real_Visit_Consent.html 1항·하단). 수집 항목 변경 시 동의 문구 + privacy.html §1 + 버전 이력 함께. **국외 이전 5요소(Google LLC)**: privacy.html §4 ↔ 서면 동의서 2항.
...
- [ ] **방문자 서면 동의서 법무 확정**: `ThinQ_Real_Visit_Consent.html` v0.9를 법무팀 검토 후 v1.0 확정. "법무팀 검토용 초안" 표기는 운영자 지시로 서식에서 제거됨(2026-08-31 — 타이틀은 "서식 v0.9"만). 워드 편집본은 운영자에게 전달됨(2026-08-31 — 리포 미포함, HTML이 원본). **3항(현장 대화 녹음 — 선택)을 실제 운영에 쓰는 시점에는 privacy.html v1.3 개정(수집 항목·목적·보유 추가) + FieldVoice `consent` 기록 방식과 정합 필요** — 3항 문안의 "요약 작성 완료 후 지체 없이 파기"는 FieldVoice 운영 기준과 대조해 확정할 것. 서명지(종이) 보관·파기 절차는 담당자 확인 필요 (시스템 미경유 — 보유 3년). **녹음(3항)의 필수 통합 여부는 법무 판단 사안** — 운영 세션 권고는 분리 유지 (선택 동의를 필수에 묶는 포괄 동의는 법 제22조 위반 소지: 녹음은 방문에 필수적이지 않음).
- [x] ~~동의서 관리자 연동~~ — **완료(2026-08-31, 운영자 지시로 선구현)**: 예약 상세 모달 [🖨 동의서] → 방문일·회사·방문자 명단 프리필 (도슨트 설명 후 서명만 받는 흐름). **법무 확정으로 서식이 바뀌면 서식·프리필 매핑 재수정 전제.** 같은 날 운영자 결정으로 3항(녹음)의 **"(선택)" 워딩 전부 삭제** — 거부권·미녹음 안내 문구("동의하지 않으시는 경우…녹음하지 않으며 불이익 없음")는 유지. 선택 동의 구분 표기 요건(법 제22조) 충족 여부는 법무 검토 확인 포인트.
- [ ] **Option B — 리포트 전월 대비 증감 표시**: 합의된 후속 작업, 미착수.
- [ ] **사내 이관 외주 견적 (오큐브) 진행 중** (2026-09-17): 팀장 지시로 「잔여 이관」/「유지보수」 분리 견적 문의 → 협력사 6개 문의에 회신(A/B 시나리오 요청). 운영 세션 진단: 코드 이관은 완료, 잔여는 사내 절차(자원 신청 4·BE팀 대기 3·이행 1) — 외주 가치는 전환 검증·유지보수. 견적 수신 후 팀장 결정 → history.md 후속 기록. 상세는 history.md 2026-09-17.
