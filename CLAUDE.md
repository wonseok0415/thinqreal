...
├── ThinQ_Real_Visit_Consent.html # 개인정보 수집·이용 동의서 (현장 인쇄용 — 서식 V1.0 법무 검토 확정, 2026-09-20 반영)
...
- [x] ~~방문자 서면 동의서 법무 확정~~ — **완료(2026-09-20)**: 법무팀 검토 확정본(Fin 2026-07-15) 수령 → 서식 V1.0으로 전면 교체. **확정본에는 구 2항(국외 이전)·3항(현장 대화 녹음) 조항이 없음** — 국외 이전은 사내 이관으로 근거 소멸, 녹음 동의는 법무 확정 범위에서 제외됨. **FieldVoice 현장 녹음을 실제 운영하는 시점에는 녹음 동의 체계(동의서 조항 신설 또는 별도 동의서) + 처리방침 개정을 법무와 별도 확정할 것** (구 v0.9 3항 문안·분리 동의 권고는 history.md 2026-08-31 기록 참조). 서명지(종이) 보관·파기 절차는 담당자 확인 필요 (시스템 미경유 — 보유 3년).
- [ ] **개인정보 문서 V3.0 게시 후속 확인 (2026-09-20 반영)**: ① 법무 검토본의 수집 항목에 **신청자 이메일·임직원 인증 이메일·방문 후기 설문(작성자 성명·소속)이 포함되어 있지 않음** — 현행 폼·인증·설문은 이들을 계속 수집하므로, 법무에 항목 추가를 확인하거나 사내 이관(SSO 전환) 시 수집 중단 시점을 확인할 것. ② 새 방침 §7(국외 이전 없음)은 개인정보 저장이 사내로 이전된 상태를 전제 — **머지=즉시 라이브 게시**이므로 Google Sheets 저장이 지속되는 동안의 게시 시점 정합은 운영자·법무 판단으로 결정.
