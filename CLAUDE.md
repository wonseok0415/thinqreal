...
├── privacy.html                  # 개인정보 처리방침 (게이트 밖 열람 가능 의도, 현재 V3.0 법무 검토 확정본 — 2026-09-20 반영)
├── ThinQ_Real_Visit_Consent.html # 개인정보 수집·이용 동의서 (현장 인쇄용 — 서식 V1.1: 1항=법무 확정 V1.0 + 2항=현장 녹음 동의(운영자 지시 유지, 법무 검토본 범위 밖))
...
`AUTH_SECRET` / `MONTHLY_REPORT_TO`(리포트 수신자) / `MONTHLY_REPORT_TEST_TO`(테스트 발송 수신자 — §8-6) / `TELEGRAM_BOT_TOKEN`·`TELEGRAM_CHAT_ID` / `CALENDAR_ID` / `SERPER_API_KEY` / `GOOGLE_CSE_ID`·`GOOGLE_CSE_KEY`(현재 403 차단) / `SURVEY_CAS_JSON`(채널 단가 — **유일한 위치**) / `FC_API_KEY`(FieldCheck 장비 키 — 2026-07-30 이전, rig config.json과 동일 값) / `monthly_report_last_sent_month`(자동 발송 가드 — §8-6 건너뛰기 체크 시에도 기록) / `roi_report_snapshot_id`(리포트 ROI 반영 지정 스냅샷 — 2026-08-24, 미설정 시 고정 수치) / `manual_sent_<YYYY-MM>`·`send_token_*`·`test_token_*`(§8-6 수동 발송 이력·일회용 토큰 — 자동 생성)
...
- **예약 확정 메일**(buildConfirm*): 📅일정/📍위치/📶Wi-Fi(2.4G·5G 분리)/🔐도어락/🅿주차/☎문의/📖안내. **민감 정보는 확정 메일에만** — SSID `ThinQ_REAL_2.4G`/`ThinQ_REAL`, PW `real2026`, 도어락 PIN `910910` (2026-09-14 교체). 값 변경은 buildConfirm* 빌더에서만.
...
- **월간 운영 리포트**: 매월 **첫째 수요일** 08:30 KST에 **전월 리포트** 자동 발송 (2026-07-29 변경 — 매일 트리거 + `isFirstWednesdayOfMonth` + 월 중복 가드. 가드는 자동만 차단).
  - **구성 (2026-08-03 §8-7 개편 + 같은 날 렌더 리뷰 반영)**: ① Executive 요약(KPI **3카드** — 라벨→값→**26년 누적** 보조줄. MoM 표기 폐기 — 2026-08-04 팀장 리뷰. **NPS 풋노트**(계산식·범위 −100~+100) 카드 하단 — NPS 표시 월에만, 2026-08-31) ② 사업부별 활용(확정 기준, **실제 저장된 본부 값 기반** 건수 내림차순 — 고정 6본부 목록 폐기, 고객가치혁신부문 등 별도 표기) ③ 목적 도넛 — 사업부별 바로 아래로 이동(2026-08-31, 구 ⑤) ④ 핵심 인사이트(큐레이션, 없으면 생략) ⑤ 인상 깊은 한마디(큐레이션, 출처=응답자 소속 dept·방문자는 익명 — '인솔자 후기' 워딩 폐기). 목적 도넛 상세(**내부 렌더링 PNG** — Apps Script가 직접 그려 메일 cid 인라인 첨부·미리보기는 data URI. QuickChart 폐기 2026-08-04, 이관 결정 §⑦ 선반영·외부 의존 0. 렌더 실패 시 인라인 HTML 막대 폴백) ⑥ 관련 기사(**최대 5건**, 썸네일 카드형 — 수동 링크 우선·미달분 자동 수집 보충, ThinQ Real 한정) ⑦ **투자 대비 성과(ROI) 최하단**(소수 1자리 억원 표기 — 기본은 고정 수치: 총 투자 2.9억(구축 2.8+운영 0.1/년)·BEP 1.31년·3년 +122.4%·5년 +270.7%. **2026-08-24부터 관리자가 지정(`roi_report_pin`)한 ROI 시나리오가 있으면 그 수치+기준 라벨로 렌더** — 미지정·결손 시 고정 폴백, 「최신 스냅샷 자동 참조」는 금지 유지(roi_snapshot 저장이 공개 경로). 연간 확정 가치·실측 누적 문장은 삭제 — 2026-08-04 팀장 리뷰). 방문 이력·설문·성과 지표 섹션은 리포트에서 삭제(상세는 관리자 페이지가 소스). 블록별 try-catch 격리.
...
- **설문 요청 메일**: §자동화 참조.
- **베스트 리뷰어 축하 메일**(2026-08-22): 관리자 설문 탭 🏆 베스트 리뷰어 서브탭(월 현황·이력·당월 후보) 또는 응답 상세 모달 [🏆 베스트 발송] → `best_reviewer_send`(관리자 토큰). 월 3명(`BEST_MONTHLY_LIMIT`)·같은 응답 재발송·@lge.com 외 수신은 서버 차단. BCC = 담당자 3+팀장(jhs.kim)+운영자(`BEST_REVIEWER_BCC`). **축하 메일만 발송 — 기프티콘(모바일 쿠폰)은 별도 채널 전달, 시스템 미경유**. 사은품 문구는 발송 화면 입력(기본 `BEST_DEFAULT_PRODUCT` — 계절 변경 시 코드 수정 불필요).

## 예약 슬롯 (확정, 변경 금지 — 2026-08-31 개편본)
1회차 09:30–11:00 / 재정비 11:00–11:30 / 점심 11:30–13:30(예약 불가) / 2회차 13:30–15:00 / 재정비 15:00–15:30 / 3회차 15:30–17:00
(구 시간표 09:00 시작은 2026-08-31 운영자 지시로 개편 — 개편 전 예약 행의 slotLabel은 구 시간 그대로 보존, 관리자 표시도 저장 라벨 우선)
...
- **예약**: 달력 → 슬롯 다중 선택 → 폼. **예약 가능일 = 오늘+2일(D+2)부터** (2026-09-14 운영자 지시로 D+7→D+2 단축, 최초 도입 2026-09-01 — 달력 차단 + `booking` 서버 검증 이중. 달력 아래에 "2일 이내는 담당자 유선 문의" 안내 문구. 긴급 방문은 유선 접수 → 관리자 백필, `admin_booking_create`는 날짜 제한 없음). 슬롯 상태 4종: 예약 마감(적) / N팀 예약 중(주황, **클릭 가능 유지**) / 예약 불가(관리자 차단, 회색, 최우선) / 선택 가능. `?type=availability` → `{bookedSlots, pendingCounts, blockedSlots}` (기존 키 하위 호환 유지).
- **예약 폼** (2026-09-01 현행): **✍️ 신청자 그룹 상단**(폼 작성자 본인 — 이름·직급·이메일) → **👤 방문 책임자 그룹**(소속 본부 드롭다운+부서 — **방문 주관 부서 기준**, 사업부별 통계·캘린더 표기의 소스 / **「방문 책임자와 신청자가 동일합니까?」 질문 + 「예, 동일합니다」 체크(기본 미체크·능동 체크 유도)** — 미체크 시 책임자 이름·직급 칸 기본 노출, 체크 시 숨김·신청자 값 복사). 그룹 분리·신청자 우선 배치·질문형 전환은 2026-09-01 운영자 피드백 4건(PR #94~97) → 방문 목적(6종, 트리거) → 동적 영역(주제/방문자 명단 최대 `MAX_VISITORS`=10/활용 방안/기대 효과 — 활용·기대는 `MIN_DETAIL_LEN`=30자 이상) → 필수 동의 2종(개인정보 수집·이용/파손·분실 — 국외 이전 동의는 2026-09-20 방침 V3.0 반영으로 폐지). 담당자·연락처 필드는 삭제됨 — `name`=방문 책임자·`applicant`=신청자 "이름 직급" 저장(2026-09-01 분리), `phone`은 빈 값.
...
- **관리 탭**: 📋 예약 관리(KPI·필터·승인/거절·상세 모달·이력 추가/수정·영구 삭제·CSV 내보내기·**동의서 인쇄** — 상세 모달 🖨: 방문일·회사·안내 담당자(=방문 책임자 `name`, 2026-09-20)·방문자 명단 프리필로 서면 동의서 열기, 해시 전달이라 성명이 호스팅 로그에 안 남음, 2026-08-31) / 📊 통계 / 🚫 슬롯 제어 / 🔐 연동 계정 / 🎬 시연 시나리오 / 💡 조명 스위치 / ⚙️ 시스템 구성 / 📦 구비 가전(45개, `?type=appliances` fetch)
...
- **데이터 로딩**: stale-while-revalidate — localStorage 캐시(`thinqreal_bookings_v1`, TTL 30분) 즉시 렌더 + 백그라운드 fresh fetch. 콜드 스타트 1~3초 스피너는 정상. **설문 탭(`survey_data`)도 동일 원칙(2026-09-21)**: 인메모리 캐시 즉시 렌더 + 탭 재진입 시 백그라운드 조용한 재조회(localStorage 미사용, 중복 조회 가드) — 구 "캐시 있으면 재조회 생략"은 열린 페이지에서 새 응답이 안 보이는 혼동으로 폐기.
...
- **소유권**: 사내 LG AX 해커톤 출품 준비(「ThinQ Real AI Field Ops」)는 **별도 세션·별도 리포**(`wonseok0415/wonseok-lab`의 `hackathon/` — 인수인계 원문은 그 리포 `hackathon/HANDOFF.md`)에서 진행. **순수 추가 트랙으로 본 시스템·FieldCheck·이관 트랙을 수정하지 않는다** (역방향 동일 — 본 리포는 그 세션에 읽기 전용 컨텍스트).
- **방향 (시스템 개선 청사진)**: 현행 예약·설문·점검·리포트 시스템 위에 **AI Agent 레이어**를 얹는 제안 — 방문 준비 / 현장 인사이트 / 품질 판정 / 운영 리포트 4개 Agent, 데이터는 **예약 건(ID) 단위 연결**, AI 산출물은 Human-in-the-Loop(담당자 검토·승인) 후 사용.
- **운영 세션 시사점**: ① 예약·설문·점검 데이터가 예약 ID로 연결 가능한 구조가 전제 — 스키마·엔드포인트 변경 시 api-contract·data-schema 기록이 더욱 중요 ② 신규 기능 설계 시 이 청사진과 상충하지 않게 참고. 지원서 세부·수치는 퍼블릭 리포 특성상 여기 기재하지 않음 (HANDOFF 원문이 소스).
...
- **보유 기간 "방문예약일로부터 3년"** (2026-09-20 V3.0 — 구 "방문일로부터 3년"에서 변경): privacy.html §3 ↔ 폼 동의 문구 ↔ 서면 동의서(내역 표·하단) 3곳 동기. 수집 항목 변경 시 동의 문구 + privacy.html §1 + 버전 이력 함께. **국외 이전 조항은 V3.0에서 전면 삭제**(privacy §7 "국외로 이전하지 않습니다" + 폼 국외 이전 체크·서면 동의서 구 2항 제거) — 해외 클라우드 재사용 등으로 재도입하려면 법무 재검토가 전제, 임의 복원 금지.
...
- **커밋 전 민감 단가 grep 필수 (2026-08-03 확장)**: `grep -rn "6,220\|34,220\|114,220\|108,000\|659원\|16,126\|Hi-Teleservice\|헤이홈\|206,000,000\|258,924,080\|279,417,851\|20,493,771"` → 0건. **콤마 없는 변형(6220 등)도 의심할 것.** 커밋 금지: CS 채널 실단가, 판매량·CS 원단위, 딜·수주 실데이터, 대장 실제 과제명·금액, 보고 PPT·사내 메일, **구축비 항목별 실집행 단가·구축/구매 소계(원 단위 정밀값)**. 커밋 가능: 총액 요약(2.794억·2.886억), BEP·ROI 지표, KOSA 공표 단가(197,714원), 구축/구매 **2단 소계의 백만원 반올림**(258.9/20.5 — §8-8 팀장 확정, ROI 툴 capex 팝업 한정). ROI 툴 보호는 파일 제외가 아니라 **내용 수준**(민감 수치 미포함)으로.

### 예약·폼
- 필수 동의 2종(수집·이용/파손·분실) 미체크 차단 로직 해제 금지 — 국외 이전 동의는 2026-09-20 방침 V3.0 반영으로 폐지(구 행의 privacyConsent 'Y'는 국외 이전 포함 의미).
- 대기중 예약이 있는 슬롯은 선택 가능 유지 (거절 시 다음 신청자 확정 가능해야 함). `availability` 응답에 새 키 추가 시 기존 키를 깨지 말 것.
- 신규 예약 `name`=방문 책임자·`applicant`=신청자 "이름 직급" (2026-09-01 분리 완료 — applicant는 항상 채워짐, 공란=구행·책임자와 동일 취급). **applicant 우선 소비처(확정·거절 인사말/설문 프리필/베스트 매칭)와 name 소비처(관리자 표·캘린더 '책임자'·사업부별 통계의 division 세트)를 바꿔치기 금지.** `phone` 컬럼은 시트에서 삭제 금지(과거 행 보존).
...
- 대장 `confirmed_amount`는 **만원 단위** (ROI 툴 파이프 입력은 백만원 — 대장 상단 「확정 가치 누적」 라인이 환산 표기). **확정 금액 3원칙 (2026-08-24)**: ① 실측/추정/미상을 `amount_basis`로 구분 (공란=실측 취급) ② 추정 = 예상 규모 구간 **하한** × 기여도 — 보수 원칙, 추정 선택 시 계산값 고정(임의 상향 불가·수정하려면 실측 전환) ③ 금액 미상 확정 허용(성사 사실과 금액 확정의 분리) — 단 **ROI 반영 체크는 금액 있는 확정만**. 리포트 ROI 반영은 **지정 스냅샷 경로**: 대장 누적을 ROI 툴에 수동 반영·저장 → ROI 분석 탭 📌 카드에서 「리포트 반영 지정」 → 다음 리포트부터 그 수치+기준 라벨 (2026-08-24 — 「최신 자동 참조」 금지는 유지, 지정 해제 시 고정 수치 복귀).
...
- BEP 대표 수치는 **1.31년 (약 1년 4개월)** — 2026-08 확정 기준(구 1.65년 대체). 리포트 ROI 기본값은 `ROI_FIXED` 상수 — **「최신 스냅샷 자동 참조」 회귀 금지는 유지**(공개 저장 경로라 임의 변조 가능). 2026-08-24부터 **관리자 지정(`roi_report_pin`) 스냅샷 참조만 허용** (지정·해제는 ROI 분석 탭 📌 카드, 팀장 협의는 운영자 담당).
...
- **구 5단계 응답은 보존** (2026-08-11 확정) — 전환일(2026-08-03) 이전 응답이 5단계인 것은 정상이며 테스트 데이터가 아니다. `classifySatisfaction()`이 척도를 분리 집계하므로 통계 오염이 구조적으로 불가 → **"구 척도"를 삭제 사유로 삼지 말 것** (실제 방문객 응답 손실). 특히 `visitor_responses`의 2026-07-28 KO 4건은 실응답 — 과거 리마인드에 "테스트 4건 삭제"로 잘못 기재된 이력이 있음.
...
- [x] ~~방문자 설문 QR 포스터 부착~~ — **완료(2026-08-11 확인)**: 실운영 응답 유입으로 확인 (8/7 10건, 8/10 5건 — 전량 NPS 신척도).
- [x] ~~방문자 테스트 응답 4건 영구 삭제~~ — **항목 자체가 오류, 2026-08-11 취소**: 해당 4건(2026-07-28, 구 5단계)은 테스트가 아니라 **실제 방문객 응답**. 삭제 금지 — 상세는 §설문 제약의 척도 규칙 참조.
...
- [x] ~~방문자 서면 동의서 법무 확정~~ — **완료(2026-09-20)**: 법무팀 검토 확정본(Fin 2026-07-15) 수령 → 서식 V1.0으로 전면 교체. 확정본에는 구 2항(국외 이전)·3항(현장 대화 녹음) 조항이 없음 — 국외 이전은 사내 이관으로 근거 소멸. **같은 날 운영자 지시로 녹음 동의 조항은 서식에 유지 → V1.1** (실제 공간에서 녹음이 진행되므로 필요 — 2항으로 재배치, 구 v0.9 3항 최종 문안 그대로: "(선택)" 워딩 없음·거부권/불이익 없음 안내 유지, 명부에 「2항 녹음 동의(동의 시 ○)」 열). 명부 동의란은 **1·2항 모두 「동의 시 ○」 방식** — 1항 「동의함/동의하지 않음」 병기는 운영자 피드백(복잡)으로 폐기(2026-09-20), 거부권 고지는 1항 본문 ※ 문구가 담당. **2항은 법무 검토본 범위 밖 — 녹음 조항의 법무 검토 + FieldVoice `consent` 기록 방식과의 정합("요약 작성 완료 후 지체 없이 파기" 문안 대조)은 열린 확인 포인트.** 서명지(종이) 보관·파기 절차는 담당자 확인 필요 (시스템 미경유 — 보유 3년).
- [ ] **개인정보 문서 V3.0 게시 후속 확인 (2026-09-20 반영)**: ① 법무 검토본의 수집 항목에 **신청자 이메일·임직원 인증 이메일·방문 후기 설문(작성자 성명·소속)이 포함되어 있지 않음** — 현행 폼·인증·설문은 이들을 계속 수집하므로, 법무에 항목 추가를 확인하거나 사내 이관(SSO 전환) 시 수집 중단 시점을 확인할 것. ② 새 방침 §7(국외 이전 없음)은 개인정보 저장이 사내로 이전된 상태를 전제 — **머지=즉시 라이브 게시**이므로 Google Sheets 저장이 지속되는 동안의 게시 시점 정합은 운영자·법무 판단으로 결정. ③ 서면 동의서 2항(현장 녹음 — V1.1, 운영자 지시 유지)은 법무 검토본 범위 밖이고 **처리방침 V3.0에는 녹음 관련 수집·이용 기재가 없음** — 녹음 조항 법무 확인 시 처리방침 반영 여부도 함께 확인할 것.
- [x] ~~동의서 관리자 연동~~ — **완료(2026-08-31, 운영자 지시로 선구현)**: 예약 상세 모달 [🖨 동의서] → 방문일·회사·방문자 명단 프리필 (도슨트 설명 후 서명만 받는 흐름). **법무 확정으로 서식이 바뀌면 서식·프리필 매핑 재수정 전제.** 같은 날 운영자 결정으로 3항(녹음)의 **"(선택)" 워딩 전부 삭제** — 거부권·미녹음 안내 문구("동의하지 않으시는 경우…녹음하지 않으며 불이익 없음")는 유지. 선택 동의 구분 표기 요건(법 제22조) 충족 여부는 법무 검토 확인 포인트. → **2026-09-20 서식 V1.0 교체 완료** — 명부 프리필 매핑(성명 tds[1]·소속·직급 tds[2]) 유지 확인, 구 3항은 확정본에서 제외되어 해당 확인 포인트 종결.
- [ ] **Option B — 리포트 전월 대비 증감 표시**: 합의된 후속 작업, 미착수.
- [ ] CSE 403 자동 해소 대기 중 (풀리면 키 동작 재확인 — 단 수동 링크로 5건이 채워진 달엔 호출 안 됨).
- 보류: 담당자 카카오톡 알림(비교 검토만, 텔레그램으로 대체됨), ABOUT 영상 자동화 다양화(Veo 클립 추가 확보 시), 설문 원본 xlsx 탭본(두 워크시트 단일 파일 — 관리자 멤버가 현행 CSV+JSONL ZIP 실사용 후 "두 파일 열기 번거로움" 피드백 시 착수, 2026-08-27 보류), 설문 행 예약 ID 연결키 보강(AI 인사이트 2단계 — 해커톤 청사진 '예약 건 단위 연결' 전제).
