...
- **예약 폼** (2026-09-01 현행): **👤 방문 책임자 그룹**(소속 본부 드롭다운+부서 — **방문 주관 부서 기준**, 사업부별 통계·캘린더 표기의 소스 / 책임자 이름·직급) → **✍️ 신청자 그룹**(「책임자와 동일」 체크, 해제 시 이름·직급 별도 입력 / 신청자 이메일). 그룹 시각 분리는 2026-09-01 운영자 피드백(소속 귀속 혼선) → 방문 목적(6종, 트리거) → 동적 영역(주제/방문자 명단 최대 `MAX_VISITORS`=10/활용 방안/기대 효과 — 활용·기대는 `MIN_DETAIL_LEN`=30자 이상) → 필수 동의 3종(개인정보 수집/국외 이전/파손·분실). 담당자·연락처 필드는 삭제됨 — `name` 컬럼에 신청자 "이름 직급" 저장, `phone`은 빈 값.
