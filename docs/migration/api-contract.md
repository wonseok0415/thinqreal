...
- **컨테이너 전용 — `/healthz` 진단 확장 (2026-09-22)**: `{ok, backend, kv, pod}` — `kv`는 `shared`(Valkey 연결, 레플리카 간 인증 코드·토큰 서명 키 공유) / `memory`(KVSTORE_ADDR 미설정) / `degraded`(주소는 있으나 연결 실패 → 메모리 폴백) / `connecting`. `pod`는 응답한 레플리카 호스트명. **멀티 레플리카에서 `kv`가 shared가 아니면 로그인·토큰이 pod에 따라 어긋난다.**
- **컨테이너 전용 — 캐시 헤더 (2026-09-22)**: `/api`·`/pub`·`/healthz` 응답은 `Cache-Control: no-store`(게이트웨이·프록시·브라우저 저장 금지 — 인증 코드·가용 슬롯이 옛 값으로 보이는 것 방지), 치환 서빙되는 HTML은 `Cache-Control: no-cache` + ETag(매번 재검증, 변경 없으면 304 — 옛 페이지가 캐시에서 그대로 열리던 함정 차단). Apps Script 현행에는 대응 헤더 없음(additive).
...
| `auth_code_peek` | `email`, `kind=auth\|admin` | **ST/QA(발송 억제 환경) 전용** — OP에서는 404 `not_found` | **컨테이너 전용(2026-09-21)** — 메일이 억제된 환경에서 UAT용으로 발급된 인증 코드를 조회 `{ok, email, kind, code, pod, kv}` / 없으면 `{ok:false, error:'no_pending_code', pod, kv}`. `pod`·`kv`는 응답한 레플리카와 캐시 모드(`/healthz`와 동일 의미 — 2026-09-22 추가). LENS 로그 대신 사용 |
...
| type | 인증 | 동작 | 부수 효과 |
|---|---|---|---|
| `booking` | — | 신규 예약 행 append (id=`Date.now()` 문자열). **2026-09-01: `applicant`(신청자 "이름 직급") 필드 추가** — `name`은 방문 책임자. **D+2 버퍼 검증** (2026-09-14, 구 D+7): `date < 오늘(KST)+2일`이면 `{error:'booking_too_soon', minDate}` 거부 (달력 차단의 서버 강제 — 클라이언트는 no-cors라 응답 미확인, 달력이 1차 게이트) | 담당자 알림 메일 + 텔레그램 발송 (신청자≠책임자면 양쪽 표기) |
