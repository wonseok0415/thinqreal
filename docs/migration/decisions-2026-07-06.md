...
### 6-8. BE팀 답변 4건 — 하이브리드 에지 최종 확정, CSR 반영(OP 주소 변경), SMTP 차주 (2026-09-18 Teams, 박현정 책임)

| # | 문의 | 답변 요지 | 이관 트랙 결정 |
|---|---|---|---|
| ① | pod 아웃바운드 정식 사용 가능? | **별도 제한 정책 없음, BE팀도 동일하게 사용** — 문제없어 보임. 단 "외부에 적재되는 데이터의 민감성"은 BE팀이 평가할 영역이 아님 | 하이브리드 에지 **정식 경로로 확정**. 민감성 평가는 이관 트랙·담당자 책임: 외부 잔류 데이터 = 익명 방문객 설문·장비 점검 로그·가명화 현장 리포트 — **현행과 동일 위치·동일 범위, 신규 노출 없음**(팀장 공유 사항) |
| ② | 공식 외부(인터넷) 진입점 패턴? | **존재하나**, 인터넷 서비스로 분류되는 순간 클라우드 영향 평가 등급이 크게 달라짐. "현 서비스가 굳이 사외로 둘 필요가 있는지 모르겠다" | **외부 진입점 추진 안 함.** 하이브리드 에지는 과도기가 아니라 **최종 설계**. `/pub`·SSO 예외는 무해하므로 유지하되 회귀 계획은 폐기(문서 정정) |
| ③ | CSR 완료 시점 | **반영 완료** — 확인 요청. **기존 `kic-op-thinq-real.thinqcloud.link` URL은 제거됨** | OP 접속 주소 = **`https://thinqreal.lge.com`(사내망 전용)** 로 변경. 담당자 실측: healthz·루트 SSO·**예외 5종이 새 호스트에서도 적용되는지** 재확인. 문서·사내 문맥(internal-context §1 OP 행) 정정. lge.com이 실제 호스트(주소창 유지)로 동작 — §6의 "redirect라 주소창은 thinqcloud.link" 예상은 무효 |
| ④ | SMTP 스펙 | 정리되는 대로, **차주** 예정 | 대기 |

- 추가 요청(회신에 포함): ST sealed-secrets 암호화용 **공개 cert 파일** — 사내 PC에 kubectl·kubeconfig가 없어 `kubeseal --fetch-cert` 불가(사내 Claude 확인). BE팀이 DB secret을 만들 때 쓴 cert를 받으면 `--cert`로 오프라인 암호화 가능.
