...
> **⚠ 2026-09-15부터 매 세션 진입점은 `internal-claude-briefing.md`(+ 사내 전용 `internal-context.md`)이다.** 이 문서는 과제 A~D의 실행 절차 원문·세션 프롬프트 모음으로 유지되며, 브리핑이 지정할 때만 해당 절을 읽는다. 과제 A·B·C는 완료(B는 2026-09-14 ST `backend:"postgres"` 실측으로 공식 완료), 남은 코드 과제는 D뿐이다.
>
