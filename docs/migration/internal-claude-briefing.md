...
| i | **사외(인터넷) 접속 경로 확보** | 외부 방문객 휴대폰 QR 설문(`/ThinQ_Real_Visitor_Survey.html`+`/pub`)이 인터넷에서 열려야 함. 점검 장비 네트워크(사내망 여부)도 확인 필요 | BE팀 답변: CSR 완료 후 `thinqreal.lge.com`이 사외에서 열리는지 / 예외 5종만 외부 노출 가능한지. **불가 시 대안**: 쇼룸 출구에 사내 Wi-Fi 태블릿을 두고 현장 작성(코드 변경 없음, 익명 유지) | 문의 발송(9/17) |
