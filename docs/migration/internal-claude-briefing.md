...
|---|---|---|---|---|
| a+b | **RDS(PostgreSQL) + ElastiCache(valkey) 신청 — DBSUPPORT JIRA 1건** | OP용 DB·캐시. ST/QA 공용 자원은 OP에 못 씀 | 티켓 1건에 RDS·ElastiCache 두 표 작성, DynamoDB 표 삭제(담당자 확인 9/21). Issue Type `DB자원(문의및검토)`, Summary `[이슈타입][AWS 계정명]…`(여백 없이, 계정명은 internal-context §2-a), Component=AWS Account, DB Engine=postgres, Description에 표 2개. 기재값은 internal-context §2-a·§2-b(**9/23 제출본으로 갱신 필요** — 담당자가 제출한 요청서 md가 단일 소스: DBMS **Aurora PostgreSQL 17.x LTS**·db.t4g.medium·Storage 20GB gp3·PRD / valkey 9.1.1·cache.t4g.micro·Shard 1·Node 2 / TAG 4종(System·Resource·Application·ThinqService)+HQ는 표 안에 기재, **추가요청 칸은 공란**, Summary 접두는 계정명 대신 조직 약칭, Instance 명은 가이드 패턴의 account 자리에 조직 약칭). ⚠ 제출본 검토 메모(9/23): ~~valkey Instance 명 끝 RDS용 접미~~ → **DB팀 확인으로 `-pgsql-aurora`만 삭제, 담당자가 `db-request-2026-09-23.md`에 반영 완료(9/23)** — internal-context §2-b는 이 수정본 기준 / Aurora는 스토리지 자동 확장이라 20GB·gp3 항목은 무의미 / ElastiCache TAG Resource가 `Database`(가이드 예시는 `Cache`) — 나머지 둘은 DB팀이 정정 요청하면 그대로 따르면 됨, 선제 수정 불필요. **제출본 파일 위치**: Gitea `docs/migration/db-request-2026-09-23.md`(사내 전용, internal-context 옆). **추가요청의 CIDR(9/22 재검토)**: 4개 대역의 출처는 Vault 가이드의 KIC-OP Allowed Bound CIDR(internal-context §2-c)이지 DB 가이드가 아니므로 **방화벽 요구값이 아니라 "참고(출처 명기)"로 적고 DB팀 표준 규칙 우선**을 명시 — 접속 주체는 "TCN EKS KIC-OP 클러스터의 thinq-real pod"로 말로 적는다. 숫자 정확성은 담당자가 Vault 가이드 원문과 대조 후 제출. **9/23 티켓 발행됨**(번호·최종 기재 수정분은 담당자 추후 전달). DB팀 1차 요청 = **서비스 개요·시스템 아키텍처 문서** → 원고 `service-overview-architecture.md`(퍼블릭, 식별자 `《…》` 공란) — **사내 Claude 작업**: 미러 pull → §8 채움 목록대로 internal-context 값 채워 Word(.docx)로 변환(구성도는 표/도형 5개) → 담당자 검토 후 JIRA 첨부. 접속 대역은 문서에 넣지 않음 | **티켓 발행·문서 준비 중** (9/23) |
...
- DBMS는 **PostgreSQL**(OP 제출본은 **Aurora PostgreSQL 17** — 와이어 호환이라 코드 무변경, `DB_HOST`에 클러스터 writer 엔드포인트), 캐시 엔진은 **valkey** — 코드가 그것만 지원. 다른 선택지는 절대 불가.
