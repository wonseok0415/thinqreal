...
- **⚠ 9/21 발견·조치**: 키트 반입 `public/`이 라이브 사본이라 ST/QA/OP 페이지가 **라이브 Apps Script를 호출**하고 있었음(전환 시점 교체 계획의 부작용). 키트 v4.1로 해소 — 컨테이너가 HTML 서빙 시 `SCRIPT_URL`을 `/api`로 자동 치환(`lib/htmlRewrite.js`), `public/`은 라이브 사본 유지. 함께 `auth_code_peek`(ST/QA 전용 인증 코드 조회 — LENS 불필요). **키트 v4.1 = 0.13.0 배포·QA 실측 통과(9/22: 시크릿 창 소스에서 `SCRIPT_URL = '/api'`, `auth_code_peek` 응답 정상) → QA 준비 완료, UAT 개시 가능.** 소스 보기는 캐시된 옛 페이지를 보여줄 수 있으니 확인은 시크릿 창에서.
