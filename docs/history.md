...
## 2026-08-04 (11) — §8-6 발송 화면 버튼 target="_top" (연결 거부 표시 수정)

- **증상**: 확인 화면에서 [나에게만 테스트 발송] 클릭 → 메일은 정상 수신되는데 화면에 "script.google.com에서 연결을 거부했습니다" 표시.
- **원인**: Apps Script HtmlService 화면은 샌드박스 iframe(googleusercontent.com)에서 렌더 — 버튼이 target 없는 일반 링크라 iframe 내부에서 script.google.com으로 이동 시도 → X-Frame-Options 거부로 결과 화면만 렌더 실패 (**요청·발송 자체는 정상 도달**). 일회용 토큰 덕에 중복 클릭도 1통만 발송됨을 확인.
- **조치**: §8-6 화면의 script.google.com 내부 링크 5곳([전체 발송]/[테스트]/재시도 2곳/레거시 안내)에 `target="_top"` — 전체 탭 이동으로 결과 화면 정상 표시. 발송 로직 무변경.
- 검증: §8-6 스텁 17항목 + 리포트 51항목 회귀. **재배포 필요** — 전체 발송 전 반영 권장.

---

## 2026-08-05 — 7월분 리포트 정식 발송 완료 (마일스톤)

- **발송**: 08:30 자동 발송 이전 오전에 수동 전체 발송 — 「자동 발송 건너뛰기」 체크로 수동본이 정식본, 당일 자동 발송은 가드(`PROP_LAST_SENT_KEY`)로 스킵. §8-6 2단계 화면·target="_top" 수정 모두 정상 동작 확인.
- **수신자 확대**: `MONTHLY_REPORT_TO`(Script Property)에 수신자 3명 추가 — 재배포 없이 즉시 적용, 발송 확인 화면 명단으로 검증 후 발송. (속성 변경이라 리포 변경 없음 — 기록만 남김)
- 발송본에는 §8-7 개편 + 팀장 리뷰 + 렌더 후속(도넛 내부 렌더링·DejaVu 숫자·카드 통일·웹폰트 600) 전체 반영 (PR #52~61).
- CLAUDE.md 운영 리마인드 정리: 팀장 리뷰·7월분 전체 발송 완료 처리. 잔여: QR 포스터 부착, 방문자 테스트 응답 4건 삭제, Option B(전월 대비 증감), CSE 403 대기.

---

## 2026-08-05 (2) — §8-8 ROI 분석툴 갱신 (팀장 리뷰 반영 4건)

- **A. Capex 실집행 반영**: 354 → 279.42백만원(하단 ⚠ 참조). Hero 정적 초기값 갱신 — BEP 「1년 4개월」(1.31년)·3년 +122.4%·5년 +270.7%·손익 서브 +3억 5,338만/+7억 8,138만. capex 팝업: 실집행 기준 279.4백만원, 구성 구축 258.9 + 구매 20.5백만원(2단 소계, 백만원 반올림 — 원 단위 정밀값·항목별 단가 미기재).
- **B. V_PR 기여도(W_PR) 신설**: `wPr` 입력+슬라이더(기본 100, 0~100, step 5) — V_PR 패널 kAdj 다음. `vPR = nPress×rMedia×kAdj×(wPr/100)`. 초기 V_PR 7,500만원 불변. 팝업 신설(AI홈 100%/가전 50%, Press Tour 3건 100% 적용). vpr 팝업 수식에도 ×W_PR 반영(§1 기타 정합화).
- **C·D. V_Quality 전면 교체 + 패널 신설**: 수식 `N_edge × D_verify × C_tester`(폐기: P_occ·Q_ship·C_AS — 외부 의존 변수 배제). 우측 컬럼에 V_Quality 패널 — `nEdge` 입력+슬라이더(기본 0, 0~200), D_verify 1일·C_tester 197,714원은 고정 상수 텍스트(근거 병기, 명명 상수 `D_VERIFY_DAYS`/`C_TESTER_DAILY`). 연간 합계 덧셈 산입(초기 +0, 2억 1,400만원 불변), 요약 스트립 「품질 가치」 `#sum-quality` 동적 금액(0원만 muted). vquality 팝업 전면 재작성 — **CS 채널 실단가·출장점검료 서술 전량 삭제**, est_value 병기·ROI 미산입 참고 표기.
- **기타 정합화**: 푸터 2026년 5월 → 8월. 가치 비중 도넛 차트에 「품질 가치」 5번째 슬라이스(#1B6CA8) 추가 — vQuality 산입 시 차트 비중 왜곡 방지(스펙 무언급, 정합 목적). 스냅샷: SIMPLE_INPUTS·collectOutputs에 wPr/nEdge/vQuality 추가, applyInputs에 §8-8 이전 구 스냅샷 폴백(wPr→100·nEdge→0, 기존 stage 폴백 패턴 준용). admin `ROI_BUILD` '20260805a' 상향(리포 규칙 — iframe 캐시 무효화, ROI 툴 외 파일이지만 필수 세트).
- **⚠ 스펙 대비 변경 2건**:
  1. **capex 기본값 279.42(step 0.01)** — 스펙 A는 279.4(step 0.1)이나, 279.4로는 ROI가 +122.5%/+270.8%로 계산되어 AC 1의 확정치(+122.4%/+270.7%, 월간 리포트 ROI_FIXED와 동일)와 불일치. 실집행의 백만원 둘째 자리 반올림(279.42)로 AC 1·2를 동시 충족 (스펙 A 「step 0.1」 조항 대비).
  2. **원 단위 실집행 금액 미기재** — 스펙 A 본문은 capex 팝업에 원 단위 정밀값 표기를 지시하나, 해당 값 자체가 AC 7의 §6.5 grep 금지 패턴에 포함(퍼블릭 리포 보호). AC 7 우선 — 백만원 반올림(279.4) 표기로 대체 (스펙 A 팝업 본문 조항 대비).
- CLAUDE.md §6.5 규칙 갱신: 구축/구매 2단 소계는 **백만원 반올림에 한해** 커밋 가능(§8-8 확정), 원 단위 정밀값은 금지 유지.
- 검증: Playwright 36항목(AC 1~5·8 전 항목 + 팝업 5종 렌더 + 폐기 서술 부재) + AC 6·7 grep 0건. **Apps Script·시트 무변경 — 재배포 불필요**, GitHub Pages 반영만 (관리자 iframe은 ROI_BUILD 상향으로 캐시 무효화).

---

## 2026-08-05 (3) — ROI 툴 후속 2건: D_verify 실측 입력화 + 관리자 동작 원리 섹션 정합

- **D_verify 입력화 (⚠ 스펙 대비 변경)**: §8-8 스펙 D는 「고정 상수 — 입력구 없이 텍스트 표기」였으나, 운영 실사용 확인(결함 발견·검증 모두 ThinQ Real이 직접 수행 → 건별 투입 일수 실측 가능)으로 입력값이 임의 추정이 아닌 실측치가 됨 — 임의 조정 방지라는 고정의 취지가 실측 원칙으로 대체 가능해 입력+슬라이더로 전환 (운영자 결정, 2026-08-05). 가드레일: 기본 1·min 1·계산부 `Math.max(1, …)` 이중 하한, 팝업에 「실측 기록 기반 입력·임의 상향 금지」 명기. `linkedPairs`/`defaults`/`SIMPLE_INPUTS`/구 스냅샷 폴백(→1) 등록. `D_VERIFY_DAYS` 상수 폐기.
- **관리자 「분석 툴 동작 원리」 정합**: §8-8 구현 시 누락 발견(사용자 지적 — 해당 섹션이 ROI 툴이 아닌 admin HTML에 있어 단일 파일 grep에서 빠짐). 연간 창출 가치 수식에 +V_Quality, V_PR 수식·변수 목록에 ×W_PR, V_Quality 설명 카드 신설(tag-blue), 해석 가이드에 Quality 조건부 산입 반영. `ROI_BUILD` '20260805b' 상향.
- 검증: Playwright 42항목(기존 36 + dVerify 초기값·연동(10건×2일=3,954,280원)·하한 클램프·리셋·구 스냅샷 폴백·팝업) + AC 6·7 grep 0건. **재배포 불필요** (Apps Script 무변경).

---

## 2026-08-05 (4) — 운영 수정 3건: 가치 항목 명칭 개편 · 확정 메일 이용안내 딥링크 · 설문 상세 모달 스크롤

- **① 가치 항목 한글 명칭 개편 (전 지점)**: R&D 효율화→**R&D 기여 가치** / 영업 지원(인프라)→**영업 기여 가치** / 영업 지원(기여이익)→**수주 기여 이익** (구현 중 워딩 확정 — 산출 구조가 명칭에 드러나도록) / PR 가치→**홍보 노출 가치** / 품질 가치→**품질 개선 가치**. 반영 3곳 — ROI 툴(요약 스트립·패널 h2·팝업 제목·차트 라벨·본문 21건), admin 동작 원리 카드(6건), gs `ROI_VALUE_LABELS`(라벨 갱신 + **vQuality 항목 추가** #1b6ca8 — collectOutputs 키 매칭 규칙 유지). 심볼(V_R&D 등)은 수식 표기라 불변. CLAUDE.md 동기화 지점에 명칭 3곳 동기 규칙 추가. `ROI_BUILD` '20260805c'.
- **② 확정 메일 「방문 전 이용 안내 페이지 열기」 홈 낙착 수정**: 메일은 `#page-guide`로 링크하나 index에 해시 라우팅이 없어 항상 홈 표시. **index에 `applyHashRoute()` 신설** — `#page-guide`·`#guide` 축약형 지원, hashchange 대응, 미존재 해시는 무시(홈 유지), 게이트와 독립 동작. **gs `GUIDE_URL` 무변경** — 이미 발송된 확정 메일의 링크도 이 수정으로 소급 동작.
- **③ 관리자 상세 모달 잘림 수정**: 기반 `.modal`에 `max-height:calc(100vh - 48px); overflow-y:auto` 부여 (`.fmodal`에만 있던 한계를 공통화). 설문 응답 상세 등 긴 내용이 스크롤로 전부 열람 가능 — 짧은 모달은 영향 없음.
- 검증: ROI 스텁 42항목 회귀 + 신규 17항목(명칭 표시·구 명칭 부재 / 해시 4경로+hashchange / 모달 computed style) + 민감 grep 0건. **gs 변경(ROI_VALUE_LABELS)은 배치 재배포 대기 목록에 포함** — 웹 엔드포인트 아님(상수)이라 지금 미배포여도 무해, 다음 재배포에 자연 합류.

---

## 2026-08-06 (1) — NPS 0~10 버튼 균일 폭 수정 (두 설문 폼 공통)

- **증상 (사용자 발견 — 방문자 설문 V1)**: 모바일(QR 스캔) 화면에서 0~9는 한 줄에 들어가고 「10」만 다음 줄로 넘어가며 잔여 줄 전체 폭으로 확장 — flex `flex:1` + `flex-wrap` 조합의 구조적 결과. "10점을 눌러달라는 유도 버튼"으로 오독될 소지 (익명 고객 대면 폼이라 중립성 훼손 리스크).
- **수정**: 인라인 flex 스타일 22곳 제거 → CSS 그리드 클래스 `.opt-group.nps`로 전환. 넓은 화면 `repeat(11,1fr)` 한 줄, ≤639px `repeat(6,1fr)` 2줄(0–5/6–10) — **줄바꿈 여부와 무관하게 전 버튼이 항상 동일 트랙 폭**. 두 폼 동일 적용 (운영 설문도 같은 구조 잠복 — NPS 전환 2026-08-03 이후 공통).
- **운영 설문 추가 조치**: `.opt-group` 기본 들여쓰기(margin-left 28px) 때문에 모바일 버튼 폭이 39px까지 축소 → 모바일에서만 NPS 그룹 들여쓰기 해제(+눈금 라벨 `.nps-hint` 클래스화로 동반 정렬), 390px 기준 44px 터치 타깃 회복.
- 마크업·저장값 무변경 (라디오 name/value·선택 JS·검증 로직 그대로 — 레이아웃만 교체). Apps Script 무변경 — **재배포 불필요**, GitHub Pages 반영만.
- 검증: Playwright 16항목 — 두 폼 × 모바일 390px/데스크톱 900px에서 11버튼 폭 편차 ≤0.02px·행 수·터치 타깃·「10」 선택 하이라이트·저장값 "10" 확인. 민감 grep 0건.

---

## 2026-08-11 — 미결 항목 실사 + 잘못된 삭제 리마인드 정정 (기록 정정)

- **경위**: 세션 재개 시 미결 항목 상태를 리포(git)·시트 실측으로 대조. 결과 4건 중 1건 완료, 1건은 **항목 자체가 오류**로 판명.
- **① QR 포스터 부착 — 완료 확인**: `visitor_responses`에 실운영 응답 유입 확인 (2026-08-07 10건, 08-10 5건, 전량 NPS 신척도 9~10점). 리마인드 완료 처리.
- **② 「방문자 테스트 응답 4건 영구 삭제」 — 항목 취소 (잘못된 리마인드)**: 대상 4건(response_id `1785200800450`/`1785200930248`/`1785201037806`/`1785201422162`, 2026-07-28 KO, "5 - 매우 만족")은 QR 테스트 데이터가 아니라 **실제 방문객 응답**. 구 5단계인 이유는 단순히 **NPS 0~10 전환(2026-08-03) 이전 응답**이기 때문 — 척도가 구형인 것이 테스트의 증거가 아님. 실행됐다면 실응답 4건 소실이었음.
  - 통계 오염 우려도 근거 없음: `classifySatisfaction()`(gs:1319)이 `/^(10|[0-9])$/`(신) / `/^([1-5])\s*-/`(구)로 분리해 평균·NPS를 각각 계산 — 섞일 구조가 아님.
  - CLAUDE.md 조치: 운영 리마인드에서 해당 항목을 취소 처리(이력 보존을 위해 삭제 대신 사유 명기) + §설문 제약에 **「구 척도를 삭제 사유로 삼지 말 것」** 규칙 신설.
- **③ ROI 라벨 재배포 — 불필요 (잘못된 경고 철회)**: 세션 초반 "9/2 8월 리포트 발송 전 `ROI_VALUE_LABELS` 재배포 필요"로 안내했으나 오판. (a) 리포 `.gs`에 이미 신 명칭 반영·구 명칭 잔존 0건, (b) **`ROI_VALUE_LABELS`는 gs:1206 정의뿐 소비처가 없음** — 리포트 ROI 섹션은 `ROI_FIXED` 고정 문구를 사용. 라벨 상수는 ROI 툴 HTML과의 동기화 참조용이라 미배포가 리포트 출력에 영향 없음. 마감 기한 없음.
- **④ 잔여 미결**: Option B(전월 대비 증감) 미착수 — 실질적 유일 잔무. CSE 403은 외부 상태 대기.
- ⚠ 기록 교훈: 리마인드 항목이 **왜 그렇게 적혔는지의 근거**(여기선 "구 척도=테스트"라는 추정)를 남기지 않으면, 후속 세션이 그 추정을 사실로 승계해 파괴적 작업을 제안하게 됨. 삭제성 리마인드에는 판별 근거를 함께 적을 것.
- 문서만 변경 — 코드·시트·Apps Script 무변경. 재배포 불필요.

---

## 2026-08-18 (1) — 관리자 추가: 권영섭 (jason.kwon)

- **`AUTH_ADMIN_EMAILS`에 권영섭(`jason.kwon@lge.com`) 추가** — 6명 → **7명**. 박광수와 동일 유형: 관리자 페이지 접근만 부여, `ADMIN_EMAILS`(담당자 알림 3 To)·`CC_EMAIL`·설문 초대 CC는 무변경 (알림 미수신 의도).
- 영구 명단 추가가 맞는 케이스 (상시 운영 인원) — `AUTH_TEMP_ADMINS` 아님. 문서 동기: CLAUDE.md 명단·인원수 3곳, api-contract.md 관리자 판정 라인(구 「영구 5명」 표기가 박광수 추가 이후 미갱신 상태였던 것도 함께 정정 → 7명).
- 월간 리포트 수신 여부는 코드와 무관 — `MONTHLY_REPORT_TO` Script Property 소관 (기등록 여부는 다음 발송 확인 화면에서 검증).
- **⚠ 재배포 필요**: 인증 판정은 웹 엔드포인트(doPost) 경로 — 배포 관리→편집→새 버전 반영 전까지 권영섭 로그인 불가. 현재 재배포 대기분: `ROI_VALUE_LABELS` 상수(2026-08-05) + 본 건.
- **재배포 완료 (2026-08-18)**: PR #68 머지 후 사용자가 배치 재배포 수행 — 실배포본에 `ROI_VALUE_LABELS`·권영섭 명단까지 반영, **재배포 대기분 0건**.
- **날짜 정정 (같은 날 후속 커밋)**: 본 항목·`.gs` 주석·api-contract의 「2026-08-06」 표기는 세션 연속 착각으로 잘못 기입된 것 — 실제 작업일 2026-08-18로 정정 (NPS 버튼 수정 08-06 (1) 항목은 실제 08-06 작업이라 유지).

---

## 2026-08-22 (1) — 베스트 리뷰어 사은품 발송 기능 (매월 3명, 9월 시행)

- **배경**: 설문 요청 메일에 「매월 베스트 리뷰어 세 분 사은품」을 공지해 왔으나 미이행 상태 — 9월부터 시행 결정 (사은품: 스타벅스 아이스 카페 아메리카노 T 2잔, 1만원 이하·계절별 변경 가능).
- **운영자 결정 2건**: ① **축하 메일만 발송** — 기프티콘(모바일 쿠폰)은 기프티쇼·카톡 등 별도 채널로 전달 (금전 가치물이 시스템을 거치지 않아 분실·재발송 리스크 0, 쿠폰 코드·바코드 미저장) ② **BCC = 담당자 3명 + 팀장(jhs.kim) + 운영자** (`BEST_REVIEWER_BCC` — 발송 사실 공유용, 수신자에게 미노출).
- **백엔드 (.gs)**: `best_reviewers` 시트 신설(`BEST_HEADERS` 10컬럼 — id/month/response_id/name/dept/email/visit_date/product/sent_at/sent_by, month는 텍스트 강제+`insightMonthKey` 정규화 재사용). POST `best_reviewer_send`(관리자 토큰) — 가드: 같은 response_id 재발송 차단 / 월 3명(`BEST_MONTHLY_LIMIT`) 초과 차단 / 수신 @lge.com 한정. **메일 발송 성공 후에만 이력 기록** (실패 시 재시도 가능). `survey_data` 응답에 `bestReviewers` 추가. 메일 빌더 3종(subject/text/html) — 다크 올리브 카드·인라인 스타일, 사은품 문구는 요청 파라미터(`product`, 기본 `BEST_DEFAULT_PRODUCT`).
- **관리자 UI (설문·대장 탭)**: 응답 상세 모달에 [🏆 베스트 발송] 버튼(발송 완료 시 비활성 「🏆 발송 완료」) → 발송 모달(입력 폼 — 백드롭 미바인딩): 선정 월(방문일 프리필)+월 현황 N/3(명단 표시·3/3 적색) / 수신 이메일(**예약 이력 자동 매칭** — 방문일+작성자명 일치 예약의 신청자 이메일, 실패 시 직접 입력 힌트) / 사은품 문구(기본값). 발송 후 no-cors 한계 보완 — **이력 재조회로 발송 성공 검증** (서버 가드에 걸리면 행 부재 → 경고 토스트). 응답 테이블에 🏆 발송 완료 마커.
- 문서 동기: api-contract(POST `best_reviewer_send` + `survey_data` 응답 확장 + 요약 카운트 GET 18/POST 28 갱신 — 구 16/17은 stale), data-schema(`best_reviewers` 10컬럼), CLAUDE.md(시트 목록·메일 규칙·분석 탭·설문 제약).
- 검증: 27항목 — 메일 빌더 단위 12(월 라벨·제목·빈 줄 보존·방문일 조건부·escape·인라인 스타일만) + 관리자 E2E 15(가짜 토큰+fetch 스텁: 프리필·자동 매칭·@lge.com 차단·월 한도 차단·정상 발송 페이로드·모달 자동 닫힘·재발송 방지·🏆 마커). 민감 grep 0건.
- **⚠ 재배포 필요**: 신규 웹 엔드포인트(`best_reviewer_send`) — 배포 관리→편집→새 버전 반영 후 사용 가능.

---

## 2026-08-22 (2) — 설문·대장 탭 서브탭 3분할 (A안)

- **배경**: 탭 하나에 워크플로 5종(응답 열람+베스트 발송 / 대장 관리 / 이슈 관리 / 방문자 열람 / 리포트 큐레이션)이 세로로 쌓여 복잡 — 운영자 지적 후 A안(탭 내 세그먼트 컨트롤) 채택. 특히 리포트 큐레이션은 설문 데이터 관리가 아닌 월간 리포트 준비 작업이라 이질적이었음.
- **구현**: KPI 5장은 공통 상단 유지, 섹션들을 3개 뷰로 래핑 — ① 📝 응답·대장·이슈(파생 관계 세트라 한 뷰 유지 — "응답 보고 대장 확인" 흐름 보존) ② 🙋 방문자 ③ 📑 리포트 준비. `.sv-seg` 세그먼트 컨트롤(Apple 스타일) + `svSubTab()` 표시 전환. **renderSurveyTab은 무변경** — 항상 전 섹션 렌더, 뷰는 display만 전환 (숨김 뷰 데이터도 항상 최신).
- **필터 스코프 정리**: 툴바를 세그먼트와 한 줄로 통합 — 트랙 필터는 main 뷰에서만 표시(응답 전용), 월 필터는 리포트 준비에서만 숨김(큐레이션은 자체 월 선택기 — 스코프 혼동 해소). 마지막 뷰는 localStorage `thinqreal_admin_sv_subtab` 기억.
- 검증: 서브탭 Playwright 9항목(기본 뷰·전환 3종·필터 표시 스코프·리로드 복원·숨김 뷰 렌더 유지·복귀) + 베스트 리뷰어 27항목 회귀 PASS. HTML만 변경 — **재배포 불필요** (같은 PR #71의 .gs 변경분은 별도로 재배포 필요).

---

## 2026-08-22 (3) — 🏆 베스트 리뷰어 서브탭 분리 (서브탭 4분할)

- **배경**: (2)의 3분할에서 베스트 리뷰어 기능이 응답 상세 모달 안 버튼으로만 존재 — 운영자 의도는 방문자·리포트 준비처럼 **별도 서브탭** 분리였음 (머지 후 확인되어 후속 PR로 반영).
- **구현**: 🏆 베스트 리뷰어 서브탭 신설 (방문자와 리포트 준비 사이) — 자체 월 선택기(`bestTabMonth`, 기본 당월·로컬 getter) + 「이 달 발송 N/3」 현황(3/3 적색) / **발송 이력 테이블**(발송 시각·이름·부서·방문일·사은품·발송자) / **당월 후보 테이블**(그 달 설문 응답 — 만족도·자유 의견 요약 표시, 행 클릭=응답 전문, [🏆 선정·발송] 버튼 → 기존 발송 모달, 발송 완료는 배지). `renderBestSection()` 신설, 상세 모달 [🏆 베스트 발송] 버튼은 보조 경로로 유지.
- 필터 스코프: best 뷰도 report처럼 상단 공용 트랙·월 필터 숨김(자체 월 선택기). localStorage 서브탭 값에 `best` 추가.
- 검증: 서브탭 13항목(4뷰 전환·필터 스코프·best 월 현황·후보 버튼·리로드 복원) + 베스트 리뷰어 27항목 회귀 PASS. HTML만 변경 — **재배포 불필요**.

---

## 2026-08-24 (1) — 대장 확정 금액 추정 산입 — "ROI가 변동해야 분석툴의 의미가 있다"

- **배경 (운영자 지적)**: 확정 모달이 확정 금액을 필수 강제 → 금액을 모르는 성사 과제는 영원히 후보에 머물거나 임의 추정치를 입력하게 됨. 설문이 예상 규모를 구간으로·기여도를 %로 받는 이유 자체가 "정확한 금액 없이 가치를 추정"하기 위함인데 확정 단계만 실측을 요구하는 설계 모순 — 결과적으로 ROI로 흘러가는 값이 없어 분석툴이 정적 전시물이 되는 문제.
- **운영자 결정 2건**: ① 추정 대표값 = **구간 하한** (보수 원칙 — 1~10억→1억·10~50억→10억·50억 이상→50억. '1억 미만'은 하한 0이라 자동 추정 불가→실측 유도) ② 활용 범위 = 리포트까지 연동 지향.
- **구현**: 확정 모달에 「금액 산정 방식」 라디오(실측/추정) — 추정 선택 시 하한×기여도 계산값 자동 채움+**readonly 고정**(임의 상향 방지, 수정하려면 실측 전환), 근거 힌트 표시, 추정 불가 시 라디오 비활성+사유. 금액 공란 저장 = **미상 확정** 허용(confirm 게이트, 재진입 보완 가능). `LEDGER_HEADERS` 끝에 `amount_basis`(실측/추정/미상 — 공란=실측 취급) + `handleLedgerUpdate` EDITABLE 등록. 대장 표: 추정 건 「(추정)」·미상 건 「미상」 표기, **ROI 반영 체크는 금액 있는 확정만 활성**. 대장 상단 **「확정 가치 누적」 라인** 신설 — 실측+추정 분리 합계, 백만원 환산(ROI 툴 수주 기여 이익 입력 참조), 미상 건수 표기.
- **⚠ 리포트 연동 유보 (제약 충돌)**: 운영자는 「리포트까지 연동」을 선택했으나, 리포트 ROI 블록은 `ROI_FIXED` 고정 표기·저장 시나리오 참조 회귀 금지가 **2026-08-04 팀장 리뷰 확정 제약**. 자동 연동은 팀장 재협의 전제로 유보하고, 이번 구현은 「대장 누적 → ROI 툴 수동 반영 → 재계산 → 팀장 승인 후 ROI_FIXED 상수 갱신」의 거버넌스 경로로 설계 (CLAUDE.md 제약에 명문화).
- **부수 수정**: 확정 일자 기본값의 `toISOString().slice(0,10)` → 로컬 getter 조합 (KST 오전 9시 이전 확정 시 전날로 밀리던 잠복 버그 — 날짜 규칙 위반 정정).
- 검증: 19항목(스키마 2 + E2E 17 — 합계 분리·표기·ROI 게이트·추정 기본 선택·readonly·실측 전환·추정/미상 저장 페이로드·로컬 today·낙관적 합계 갱신) + 서브탭 13·베스트 리뷰어 27 회귀 PASS.
- **⚠ 재배포 필요**: `handleLedgerUpdate` EDITABLE 변경 (doPost 경로) — 미배포 시 amount_basis가 시트에 저장되지 않음 (기존 필드는 정상 동작).

---

## 2026-08-24 (2) — 리포트 ROI 블록 동적 반영 — 지정(pin) 스냅샷 경로

- **배경 (운영자 결정)**: (1)에서 유보했던 「리포트까지 연동」 확정 — "ROI가 변동하지 않으면 분석툴의 의미가 없다". 팀장 협의는 운영자 담당.
- **설계 — 지정(pin) 경로**: `roi_snapshot` 저장은 토큰 없는 공개 경로라 「최신 스냅샷 자동 참조」(2026-08-04 폐기된 구 설계)로 회귀하면 URL만 아는 누구나 리포트 수치를 바꿀 수 있음 → **관리자가 명시 지정한 스냅샷만 리포트가 참조**. Script Property `roi_report_snapshot_id` + POST `roi_report_pin`(관리자 토큰, 미존재 id 거부, `id:''`=해제). **미지정·조회 실패·필드 결손은 전부 `ROI_FIXED` 고정 수치 폴백** — 지정 전까지 리포트는 현행과 완전 동일 (머지·재배포가 팀장 협의를 선점하지 않음).
- **.gs**: `resolveReportRoi(allRoi)` — 지정 스냅샷의 inputs(capex/opex 백만원)·outputs(roi3/roi5/bepYears)를 리포트 표기 형식으로 변환(`toEokStr` 억원 1자리·`bepTextFromYears` "1.31년 (약 1년 4개월)"·± 퍼센트) + 기준 라벨(`basis` — 시나리오명·저장일). collect의 `roiFixed`가 지정 우선·고정 폴백. 텍스트·HTML 빌더에 기준 라벨 줄 추가(미지정 시 표기 없음 — 기존 렌더 무변화). GET `roi_snapshots` 응답에 `reportPinnedId` 추가(비민감).
- **관리자 UI**: ROI 분석 탭 iframe 아래 「📌 월간 리포트 반영 시나리오」 카드 — 시나리오 목록(최신순, ISO 라벨은 날짜 변환)·현재 지정 상태·[리포트 반영 지정]/[지정 해제(고정 수치)]. no-cors 한계 보완 — 지정/해제 후 재조회로 성공 검증.
- 운영 흐름: 대장 「확정 가치 누적」 → ROI 툴 수주 기여 이익 반영·시나리오 저장 → 📌 지정 → 다음 리포트부터 동적 수치+기준 라벨. 팀장 협의 완료 전에는 지정하지 않으면 됨.
- 검증: 23항목(단위 16 — 억원/BEP/퍼센트 변환·미지정/미존재/결손 폴백·collect/라우팅/빌더 배선 + E2E 7 — 목록·지정·해제·재조회 검증) + 회귀 59항목(서브탭 13·베스트 27·대장 추정 19) PASS.
- **⚠ 재배포 필요**: 신규 엔드포인트(`roi_report_pin`)·`roi_snapshots` 응답 확장·리포트 빌더 변경 — (1)의 `amount_basis` 건과 묶어 재배포 1회 권장.

---

## 2026-08-24 (3) — 확정 모달 추정 불가 상태 UX 개선

- **배경 (운영자 리포트)**: "추정 버튼이 클릭되지 않는다" — 예상 규모 「모름」 행에서 추정 라디오가 비활성화되는 것은 의도된 동작(하한 계산 재료 없음)이나, 회색 소형 힌트만으로는 비활성 사유가 안 보여 고장으로 오인됨 (현 대장 5건 중 4건이 「모름」이라 대부분의 행에서 발생).
- **개선**: 추정 불가 시 힌트를 **경고 콜아웃**(주황 배경 `--c-orange-light`·#C2540A, ⚠ + 굵은 제목)으로 강조 + **해소 경로 안내 추가** — "[수정] 버튼에서 예상 규모를 구간 값(1~10억/10~50억/50억 이상)으로 입력 → 재확정 시 추정 활성화, 모르면 실측 또는 미상 확정". 비활성 라디오 라벨에 `cursor:not-allowed`. 추정 가능 시 힌트는 기존 회색 유지. 사용자 데이터 보간은 escapeHtml.
- 검증: LEDGER-EST 21항목 (기존 19 + 수정 경로 안내·콜아웃 배경 2) PASS. HTML만 변경 — **재배포 불필요**.

---

## 2026-08-24 (4) — 운영 기록: IoT 이슈 1건 오기입 삭제 (코드 무변경)

- `iot_issue_log`의 `1784591925173-I1` (출처: 2026-07-21 08:58 KST 제출 etc 트랙 응답 — 기기·증상 공란) — 운영자가 작성자 확인 후 **오기입으로 판단, 영구 삭제** (실수 정리용 삭제 규칙 부합. 실제 이슈였다면 기각 보존이 원칙).
- **⚠ 미래 세션 주의**: 원본 응답 `1784591925173`(survey_responses)에는 `iot_defect='발견함'`이 그대로 남아 있고 응답 테이블 ⚠ 마커도 표시됨 — **파생 실패가 아니라 파생 행만 의도적으로 삭제된 상태**. 원본 응답의 iot_defect 값을 수정하거나 이슈 행을 재생성하지 말 것 (응답 원문 보존 원칙).

---

## 2026-08-25 (1) — 운영 기록: 리포트 ROI 동적 반영 파이프라인 첫 가동 (코드 무변경)

- **전 구간 첫 실운영 완료**: 「R&D전문 공간 검토」 확정(추정 2,500만원 — 1~10억 하한 × 25%) → ROI 툴 신규 Task 기여 반영(pipe 25·margin 100·attr 100·stage 확정 — 대장 누적값 직입 방식, 마진·기여도 100%로 이중 차감 방지) → 시나리오 저장 → **📌 리포트 반영 지정 (2026-08-25)**.
- **현재 리포트 ROI 상태**: 지정 시나리오 수치 — 연간 2억 3,900만원 · BEP 1.17년(1년 2개월) · 3년 +148.4% · 5년 +314.0% + 기준 라벨. **다음 자동 발송(9/2 수 08:30, 8월분)부터 적용** — 팀장 협의 지연 시 📌 [지정 해제]로 고정 수치 복귀 가능(가역).
- 과정 중 확인된 운영 노트: ① ROI 툴 카테고리 마진 기본값(task 4.9%)이 남아 122만원으로 계산되는 실수 발생 → 마진 100 정정으로 해결 (수동 반영 시 4개 필드 전부 확인 필요) ② 순단 구간의 시나리오 저장 유실 1회 → 재저장 (저장 성공 판별 = 툴 하단 목록 등장) ③ 관리자 "데모 데이터" 화면은 일시 연결 실패 폴백임을 확인 (데이터 무손상).

---

## 2026-08-25 (2) — 기사 큐레이션 메타 추출 3종 수정 (16진 엔티티·소급 힐링·UA 폴백)

- **증상 (운영자 발견, 8월 리포트 준비 중)**: ① MTN 기사 제목 뒤쪽이 `&#x27;` 등 기호·숫자로 표시 ② Instagram 요약 전체가 엔티티 문자열 ③ chip.de 기사 제목·썸네일 추출 실패(URL이 제목 자리에 노출).
- **원인**: `decodeHtmlEntities`가 10진(`&#39;`)만 처리하고 **16진(`&#x27;`·`&#xc744;`) 숫자 엔티티 미지원** — 두 케이스 모두 이 형식. chip.de는 봇 UA(`ThinQRealBot`) 요청이 차단(비200)돼 추출 전체 포기.
- **수정 (.gs)**: ① `decodeHtmlEntities` — 16진/10진 `fromCodePoint`(astral 안전) 우선 변환 + `&amp;`는 마지막(이중 디코딩 방지) ② `enrichArticleFromUrl` — 시트에 이미 저장된 title/snippet/source에 디코딩 소급 적용 (16진 미지원 시절 저장분 자동 힐링, 평문 무변화라 안전) ③ `fetchUrlMeta` — 봇 UA 실패(비200 또는 유효 메타 0) 시 **브라우저 UA로 1회 재시도** (`fetchUrlMetaWith` 분리).
- **자동 추출 한계 (운영 안내)**: Instagram은 비로그인 접근이 로그인 랜딩으로 떨어져 일반 제목("Instagram")·홍보 문구만 추출됨 — 구조적 한계. 필요 시 `monthly_articles` 시트에서 title/summary/thumbnail 수기 입력 (채워진 필드는 보존되는 기존 설계 활용). chip.de는 UA 폴백으로 해소 기대, 실패 시 동일하게 수기 입력.
- 검증: 15항목 (16진 한글/따옴표/astral·이중 디코딩 방지·평문 무변화·og 파싱·소급 힐링·UA 폴백 배선) + `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요**: 기사 추출은 `article_add`(doPost)·리포트 미리보기(doGet) 경로 — **9/2(수) 8월분 자동 발송 전 재배포 필수** (미배포 시 발송본에도 깨진 제목·빈 썸네일 그대로 나감). 재배포 후 [리포트 미리보기]를 한 번 열면 소급 힐링·재추출이 실행됨.

---

## 2026-08-25 (3) — 기사 엔티티 소급 힐링 위치 수정 (PR #81 후속 — 미리보기 무변화 원인)

- **증상**: PR #81 재배포 후에도 미리보기의 MTN 제목이 여전히 깨짐. **원인 = 내 수정 위치 오류**: 소급 힐링을 `enrichArticleFromUrl` 안에 넣었으나, 리포트 조립부는 **title이 채워진 행이면 enrich를 통째로 건너뜀** (`baseItem.title ? baseItem : enrich…`) — 깨진 값도 "채워진 title"이라 힐링 미실행.
- **수정**: 힐링을 **읽기 지점으로 이동** — ① 리포트 조립부: 시트에서 읽은 title/source/summary를 즉시 `decodeHtmlEntities` (enrich 스킵과 무관하게 적용) + **힐링 값이 저장분과 다르면 시트에 1회성 교정 write-back** (다음 읽기부턴 평문) ② `readArticleRows`: 관리자 큐레이션 목록 표시도 동일 디코딩. URL·thumbnail은 디코딩 제외(주소 보존). enrich 내부 힐링은 무해하여 유지.
- 검증: 19항목 (기존 15 + 읽기 경로 배선 4) + `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요 (재차)** — PR #81 배포본에는 이 수정이 없음. 9/2 발송 전 반영 필수. 반영 후 [리포트 미리보기] 1회 열람 시 화면 교정 + 시트 영구 교정 동시 수행.

---

## 2026-08-26 — 운영 기록: 기사 엔티티 힐링 검증 완료 + 잔여 2건 수기 입력 결정 (코드 무변경)

- **PR #82 재배포 후 검증 (운영자 미리보기 확인)**: MTN 기사 제목 실제 교정 확인 (`&#x27;빨간불&#x27;` → `'빨간불'`) — 읽기 지점 힐링 + 시트 1회성 교정 파이프라인 정상 작동 입증. 이 건 종결.
- **잔여 2건은 자동 추출 구조적 불가 확정 → `monthly_articles` 시트 수기 입력으로 결정**:
  - **chip.de**: 봇·데이터센터發 접근을 사이트가 전면 차단 (PR #81의 브라우저 UA 폴백 포함 어떤 UA도 실패, 외부 크롤러 교차 확인과 일치). 제목은 URL 슬러그가 곧 헤드라인인 매체 특성상 「Vier Tage in Seoul mit LG: Wie der Konzern das Zuhause der Zukunft baut」로 도출 가능 — 운영자가 사내망 브라우저에서 원문 대조 후 title(·summary·thumbnail) 수기 입력.
  - **Instagram**: 비로그인 접근 = 로그인 랜딩 (게시물 내용 미노출) — 구조적 한계. 요약 말미의 `&#…` 파편은 16진 미지원 시절 저장 시 truncate가 엔티티 중간을 자른 흔적 (신규 추출은 디코딩 후 truncate라 재발 없음) — 수기 입력으로 함께 해소. 리포트 가치가 낮으면 큐레이션 UI [삭제] 후 타 기사 교체도 대안 (5건 미달분은 자동 수집 보충).
- **설계 근거**: 담당자가 채운 title/summary/thumbnail은 자동 추출이 덮지 않음 (`baseItem.title ? baseItem : enrich…`) — 수기 입력이 곧 영구 해결. 추가 코드 수정 불필요 판단.
- 다음 마일스톤: **9/2(수) 08:30 — 8월분 리포트 자동 발송** (📌 지정 활성: BEP 1.17년·+148.4%/+314.0% + 기준 라벨). 발송 전 체크: ① 기사 2건 수기 입력 후 [리포트 미리보기] 최종 확인 ② 8월 인사이트·한마디 큐레이션 입력 ③ 팀장 협의 지연 시 📌 [지정 해제]로 고정 수치 복귀 가능.

---

## 2026-08-26 (2) — 기사 [수정] 모달 신설 (article_update) + UI 설계 원칙 명문화

- **배경 (운영자 지시 3건)**: ① 큐레이션 UI가 URL 붙여넣기 전용이라 자동 추출 불가 기사(chip.de·Instagram)의 제목·요약·썸네일을 채울 칸이 없음 — 시트 직접 편집 안내는 부적합 ② **운영자 본인은 ThinQ Real 상시 운영자가 아니므로 UI는 실사용 관리자 편의 기준으로 설계할 것** (핵심 제약에 명문화) ③ 사내 이관의 데이터 저장소 이동(Sheets→사내 PostgreSQL 점진 이관 — decisions §2-⑥·§6) 인지하에 작업할 것.
- **.gs**: POST `article_update`(관리자 토큰) 신설 — month+url로 행 식별(둘은 수정 불가·행 정체성), EDITABLE 5종(title/source/summary/published_at/thumbnail)만 전송분 트림 저장. **빈 문자열 저장 = 비우기 허용**(title 비우면 다음 리포트 빌드 때 자동 추출 재시도), 미전송 필드 보존, 미존재 행 not_found. `readArticleRows`에 summary·thumbnail 추가(수정 모달 프리필용 — summary는 엔티티 디코딩, thumbnail은 주소라 제외).
- **관리자 UI**: 큐레이션 행마다 [수정] 버튼 → ✏️ 기사 정보 수정 모달(입력 폼 — 백드롭 미바인딩 원칙). 제목/출처/게재일/요약/썸네일 URL 5필드 + 썸네일 미리보기(주소 오입력 시 숨김), 썸네일 형식 가드(http(s) 아니면 저장 차단·모달 유지). 제목 미추출 행에는 주황 경고 배지 「⚠ 제목 자동 추출 실패 — [수정]에서 직접 입력」 표시. 저장 = 낙관적 반영 + 재조회 검증(no-cors 보완).
- **이관 트랙 반영**: api-contract.md에 article_update 행 + survey_data articles 행 스키마(summary·thumbnail 추가) 기재 — POST 29→30종. 저장소 어댑터 Articles 인터페이스가 이식 대상.
- **미해결 참고**: 운영자가 참고 요청한 "LG AX 해커톤" 채팅 내용은 리포·드라이브 어디에도 기록이 없어 반영 불가 (클로드 채팅 세션 간 열람 불가) — 핵심 결정이 공유되면 이 파일에 기록 예정.
- 검증: ARTICLE-EDIT 22항목 (단위 7 — 5필드 저장·트림·부분 전송 보존·빈 값 비우기·not_found 2종·정체성 불변 / 배선 4 / E2E 11 — 버튼·배지·프리필·가드·페이로드·재조회 반영) + 회귀 103항목(엔티티 19·서브탭 13·베스트 27·대장 추정 21·ROI pin 23) 전부 PASS. `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요**: 신규 doPost 엔드포인트(`article_update`) + `readArticleRows` 응답 확장 — 재배포 전에는 배포본이 article_update를 몰라 `Unknown type`으로 무시됨 (화면은 낙관적 반영 후 재조회에서 원복되므로 저장 실패가 눈에 보임).

---

## 2026-08-26 (3) — 해커톤 트랙 맥락 수령·기록 (코드 무변경)

- **경위**: 운영자가 "LG AX 해커톤" 세션 참고를 지시 → 해당 세션의 인수인계 문서 `hackathon/HANDOFF.md`는 **별도 리포**(`wonseok0415/wonseok-lab` — 본 세션 GitHub 접근 범위 밖)에 있어 리포·드라이브 검색으로는 미발견, 운영자가 채팅 첨부로 전달.
- **요지**: 사내 LG AX 해커톤 출품 「ThinQ Real AI Field Ops」 — 현행 예약·설문·점검·리포트 시스템 위에 AI Agent 레이어(방문 준비/현장 인사이트/품질 판정/운영 리포트 4 Agent)를 얹는 제안. 데이터는 예약 건(ID) 단위 연결, Human-in-the-Loop 검토 후 사용. 해커톤 트랙은 순수 추가 — 본 시스템·FieldCheck·이관 트랙 미수정.
- **기록**: CLAUDE.md에 §해커톤 트랙 신설(세션 간 동기화용 — FieldCheck·이관 섹션과 동일 패턴). 지원서 세부 내용·수치는 퍼블릭 리포 특성상 미기재 (HANDOFF 원문이 단일 소스).
- **운영 세션 반영 방침**: 신규 기능·스키마 설계 시 Agent 청사진(예약 ID 단위 데이터 연결성)과 상충하지 않게 참고. api-contract·data-schema 기록 규칙의 중요도가 상승 (Agent 레이어와 이관 컨테이너 양쪽이 이 문서를 소비).

---

## 2026-08-27 (1) — 설문 원본 내보내기 (AI 인사이트 추출 1단계)

- **배경 (관리자 멤버 요청, 운영자 경유)**: 설문 피드백 raw data를 내려받아 AI Agent로 인사이트를 추출하고 싶다는 요청. 사내 Agent 구성 확정 전이므로 **1단계 = 관리자 페이지에서 원본 데이터 확보 경로**로 합의 (2단계 후보: 설문 행에 예약 ID 연결키 보강 — 해커톤 청사진의 '예약 건 단위 연결' 전제 / 3단계: 이관 후 PostgreSQL 직접 조회).
- **UI**: 설문·대장 탭 툴바 [⬇ 원본 내보내기] → 모달(입력 폼 — 백드롭 미바인딩)에서 대상 선택(설문 응답·방문자 현장 설문, 건수 표시·기본 전체 선택) + 다운로드 사유 + 비밀번호. **전체 기간 전 컬럼**(응답은 raw_json 포함)을 CSV(BOM, 엑셀용)+JSONL(AI 분석용) 두 형식으로 한 ZIP에 담아 다운로드.
- **개인정보 절차는 예약 CSV와 동일 (게이트 재사용)**: 사유 5자+ / 비밀번호 ASCII 8자+ / `export_log` 기록 성공 후에만 다운로드(실패 시 차단·모달 유지) / 비밀번호 미기록. 감사 추적을 위해 reason에 `[설문 원본: 응답 N건 · 방문자 M건]` 접두 (export_log 컬럼 무변경).
- **ZipCrypto 다중 파일 일반화**: 기존 단일 파일 전용 `makeEncryptedZip`을 `makeEncryptedZipFiles(files, password)` 기반으로 재구성 — 엔트리마다 키 스트림 재초기화, central directory·EOCD 다중 엔트리 정렬. 기존 예약 CSV 경로는 래퍼로 동일 동작 (테스트로 왕복 검증).
- **부수 정정**: 예약 CSV 파일명 stamp의 `toISOString().slice(0,10)` → 로컬 getter(`slotDateToYMD`) — KST 오전 9시 이전 내보내기 시 파일명 날짜가 전날로 밀리던 날짜 규칙 위반 정정. `genExportPassword`는 targetId 인자 일반화(기존 호출 무변경 호환).
- 검증: SV-EXPORT 24항목 (ZipCrypto 왕복 8 — 4파일 암·복호화 원문 일치·검증 바이트·CRC·오답 비밀번호·단일 래퍼 회귀 / 빌더 5 — 헤더 합집합·이스케이프·줄바꿈·null·JSONL 왕복 / E2E 11 — 버튼·모달·사유 미입력 차단·기록 실패 시 다운로드 차단·reason 접두·파일명·ZIP 4파일·BOM·내용 왕복) + 회귀 125항목(기사 수정 22·엔티티 19·서브탭 13·베스트 27·대장 추정 21·ROI pin 23) 전부 PASS.
- **재배포 불필요**: HTML만 변경 — `export_log`는 기존 엔드포인트 그대로 사용 (서버 무변경).

---

## 2026-08-27 (2) — 운영 기록: 설문 원본 내보내기 반영 완료 + xlsx 탭본 보류 결정 (코드 무변경)

- **PR #86 머지 완료** — 재배포 불필요 건이라 머지 즉시 라이브 반영. 관리자 멤버에게 "내보내기 한 번이면 방문자 데이터까지 함께 온다"고 안내하면 되는 상태.
- **운영자 확인 문답**: "인솔자 기준인 것 같은데 방문자(QR) 결과도 함께 받아야" → **이미 포함 설계임을 확인** — 모달 대상 2종(응답·방문자) 기본 전체 선택, ZIP 안에 `survey_responses.*` + `visitor_responses.*` 4파일. 두 설문은 컬럼 구조가 달라(42 vs 11) 파일 분리가 정보상 "엑셀 탭 2개"와 동일.
- **⚠ 보류 결정 — 설문 원본 xlsx 탭본**: 운영자가 제안한 "한 엑셀 파일에서 탭 분리"는 사람 열람 편의 개선(정보 동등)이므로, **관리자 멤버가 현행 구조를 실사용한 피드백이 온 뒤에 착수**하기로 결정. 기술 경로는 확인됨(xlsx=ZIP 컨테이너 — 자체 ZIP 생성기로 두 워크시트 xlsx 생성 → 비밀번호 ZIP에 동봉). CLAUDE.md 보류 목록에 등재.
- 다음 마일스톤 변동 없음: 9/2(수) 08:30 8월분 리포트 자동 발송 (발송 전: 기사 2건 [수정] 모달로 메타 입력 → 미리보기 확인, 8월 인사이트·한마디 큐레이션).

---

## 2026-08-31 (1) — 방문자 개인정보 서면 동의서 서식 신설 (법무팀 제안 반영)

- **배경 (운영자 전달 — 법무팀 검토 의견)**: ThinQ Real은 B2E 게이트 구조라 **예약 신청자(사내 컨택 파트너)와 실제 방문 고객(외부 B2B)이 분리** — 방문자 명단은 신청자가 대리 입력하므로 **정보주체(방문 고객) 본인의 동의가 부재**. 법무팀이 현장 서면 동의를 제안 → 인쇄용 템플릿 제작 결정. (직전 개인정보 히스토리 브리핑의 어젠다 ①·② 해당 — FieldVoice 녹음 동의 검토와 같은 계기.)
- **신규 `ThinQ_Real_Visit_Consent.html`** (리포 루트, privacy.html 팔레트 계승·인쇄 A4 최적화): ① 개인정보 수집·이용 동의(필수 — 성명·소속·직급·방문 기록, 목적, **보유 3년**, 거부권·불이익 고지) ② 국외 이전 동의(필수 — privacy.html §4의 Google LLC 5요소 문안 계승) ③ **현장 대화 녹음·인사이트 활용 동의(선택 — FieldVoice 대비**: 가명처리 요약만 활용·원본 미공유·거부 시 불이익 없음/해당 세션 미녹음) + 방문자 서명 명부 10행(`MAX_VISITORS` 정합, 필수 서명란·선택 ○ 표시란 분리). 인쇄 시 **앞면 고지문 + 뒷면 명부 = 양면 1장**. 화면용 🖨 인쇄 버튼(인쇄에는 숨김).
- **서식 v0.9 — 법무팀 검토용 초안 표기**: 문안 확정 전 배포 방지 목적. 확정 시 v1.0으로 갱신(초안 문구 제거). 검증: headless 크로미엄 PDF 렌더 2페이지 확인, 운영자에게 PDF 전달.
- **후속 (운영 리마인드 등재)**: ① 법무 확정 → v1.0 ② 3항 실운영 시 **privacy.html v1.3 개정 필수**(현재 방침에 녹음 미기재) + FieldVoice `consent` 컬럼 기록 방식과 정합 ③ 3항의 원본 파기 시점 문안은 FieldVoice 운영 기준(wonseok-lab DESIGN.md)과 대조 확정 ④ 서명지 보관·파기 절차(종이, 3년)는 담당자 협의.
- 동기화 지점 추가: 보유 3년 = privacy.html §3 ↔ 예약 폼 문구 ↔ 서면 동의서 / 국외 이전 5요소 = privacy.html §4 ↔ 서면 동의서 2항.

---

## 2026-08-31 (2) — 동의서 워드 편집본 전달 + 녹음 항목 분리 유지 권고 + 관리자 연동 계획 (코드 무변경)

- **워드 편집본 전달**: `ThinQ_Real_방문자_개인정보_동의서_v0.9.docx` (HTML 서식과 동일 내용·A4 2쪽) + PDF를 운영자에게 채팅 파일로 전달 — 법무팀 회람·직접 수정용. **리포에는 미포함** (HTML `ThinQ_Real_Visit_Consent.html`이 단일 원본 — 법무 확정 문안은 HTML에 반영 후 워드 재생성).
- **녹음(3항) 필수 통합 검토 → 분리 유지 권고**: 운영자가 "녹음을 별도로 분리하지 않는 것"을 제안 → 운영 세션 권고는 **분리 유지**. 근거: 녹음은 방문·시연 이용에 필수적이지 않아 필수 동의에 묶으면 **선택 동의의 포괄·강제 동의**(개인정보보호법 제22조 위반 소지)가 되며, 서면 동의를 제안한 법무팀이 오히려 지적할 가능성이 높음. 최종 판단은 법무팀 소관 — 법무가 통합을 지시하면 서식 1회 수정으로 대응. (관리자 연동 프리필이 도입되면 분리 유지의 현장 운영 부담도 사실상 소멸.)
- **관리자 연동 계획 등재 (운영자 요청)**: 방문자 명단은 사전 확보되므로 현장 자필 기입은 시간 낭비 → 예약 상세 모달 [동의서 인쇄] 버튼으로 방문일·회사·명단(`visitors` JSON) **프리필 인쇄 뷰**를 열고, 현장에서는 도슨트 설명 후 서명만 받는 흐름. **착수는 법무 문안 확정 후** (서식 변경 시 연동 재작업 방지 — 리마인드 등재).

---

## 2026-08-31 (3) — 동의서 (선택) 워딩 삭제 + 관리자 [동의서 인쇄] 프리필 연동

- **배경 (운영자 지시 2건)**: ① 녹음(3항)의 "(선택)" 워딩 전부 삭제 ② 관리자 연동을 법무 확정 전에 현재 서식 기준으로 선구현 (문안 변경 시 재수정 전제 — 기존 "법무 확정 후 착수" 순서를 운영자가 변경).
- **동의서 워딩 (`ThinQ_Real_Visit_Consent.html` + 워드·PDF 재생성)**: 3항 헤딩 "(선택)" 태그·"※ 선택 항목입니다" 문두·명부 헤더 "(선택 — 동의 시 ○)" 3곳 삭제. **거부권·미녹음 안내는 유지** — "동의하지 않으시는 경우 안내 담당자에게 말씀해 주시면 해당 세션은 녹음하지 않으며 불이익 없음" (법적 안전장치). 1·2항 "(필수)" 표기와 명부의 3항 ○ 열 구조는 유지. ⚠ 선택 동의 구분 표기 요건(법 제22조) 충족 여부는 법무 검토 확인 포인트로 리마인드에 명시.
- **관리자 연동 (`thinqreal_admin.html`)**: 예약 상세 모달에 [🖨 동의서] 버튼 → `printConsentFromModal()` — 방문일·방문 회사(`clientCompany` 우선, 없으면 방문자 org 합집합)·방문자 명단(`visitors` JSON org/name/rank, 최대 10명)을 **URL 해시 프래그먼트(b64url JSON)**로 동의서 페이지에 전달. 동의서 페이지는 해시를 읽어 기입란·명부 프리필 (서명·○ 칸은 항상 공란). **해시 전달 이유: 쿼리스트링과 달리 서버로 전송되지 않아 방문자 성명이 GitHub Pages 로그에 남지 않음.** 해시 없음/손상 시 빈 서식 그대로 (수기 경로 보존).
- 검증: CONSENT-PREFILL 16항목 (워딩 정적 4 / 프리필 6 — 결합 표기·공란 허용·손상 해시 무해 / 관리자 6 — 버튼·페이로드·해시 방식·관리자→동의서 왕복) + 관리자 회귀 107항목(서브탭 13·원본 내보내기 24·기사 수정 22·베스트 27·대장 추정 21) PASS.
- **타이틀 "법무팀 검토용 초안" 워딩도 삭제 (운영자 추가 지시, 같은 PR)**: 부제를 "서식 v0.9"로 정리 — 워드·PDF 동기 갱신. 문안 미확정 상태 관리는 서식 표기 대신 CLAUDE.md 리마인드로만 추적.
- **재배포 불필요**: HTML 2종만 변경 (서버 무변경). 워드·PDF 갱신본 운영자 전달 완료.

---

## 2026-08-31 (4) — 운영 시간 개편: 전 회차 30분 후행 (09:30 시작)

- **운영자 지시**: 1회차 09:30–11:00 / 2회차 13:30–15:00 / 3회차 15:30–17:00로 변경 (구 09:00/13:00/15:00 시작). 이용안내 시간표는 기존 구조 유지로 도출 — 재정비 11:00–11:30·15:00–15:30(30분×2), 점심 11:30–13:30(예약 불가).
- **변경 지점 전수 (11곳)**: ① index — 예약 슬롯 카드 3종·이용안내 운영 시간표 6행·`SLOT` 라벨 맵(신규 예약 slotLabel 소스) ② admin — 백필 폼 회차 옵션·`SLOT_LABELS`·`fmtSlots` ③ gs — `SLOT_TIMES`(캘린더 일정 시각)·`SLOT_LABEL_TEXT`(캘린더 설명)·테스트 픽스처 ④ docs — CLAUDE.md 슬롯 섹션·data-schema 2곳.
- **이력 보존 설계**: `fmtSlots`가 **저장된 slotLabel 우선** 표시로 변경 — 개편 전 예약(구 시간 라벨 저장)이 새 시간으로 왜곡 표시되지 않음 (상세 모달·CSV 포함). 라벨 없는 행만 신 라벨 폴백. 통계·availability는 회차 번호 기준이라 무영향.
- **자동 반영 확인 (수정 불필요)**: 신규 예약 메일·텔레그램·알림은 모두 저장 slotLabel 사용 → index 라벨 맵 갱신으로 자동 전환. 웰컴보드 게첨 시간 예시(09:00~13:00)는 슬롯과 무관해 유지. `server/`(이관 컨테이너)는 소유권 규칙대로 미수정 — data-schema 갱신본이 이식 기준.
- 검증: SLOT-TIME 17항목 (구 문자열 잔존 스윕 3파일 0건 / 신 라벨 6곳 / 캘린더 시각 단위 — 1·3회차 일정 09:30~11:00·15:30~17:00 / fmtSlots 이력 보존 3케이스) + 회귀 146항목 PASS. `.gs` 문법 체크 PASS. (NPS-GRID 스위트는 컨테이너 재활용으로 파일 소실 — 기능은 PR #66 라이브 확인 완료라 무관.)
- **⚠ 재배포 필수 + 운영 전환 주의**:
  1. Apps Script 재배포 — `SLOT_TIMES`(확정 시 캘린더 일정 시각)·`SLOT_LABEL_TEXT`. 미배포 시 새 확정 건의 캘린더가 구 시간으로 생성됨.
  2. **개편 전에 확정된 미래 방문 건**은 확정 메일·캘린더가 구 시간 기준 — 새 시간 적용 대상이면 관리자 [수정]에서 저장(slotLabel 재산출 + 캘린더 delete+recreate로 신 시간 반영) 후 예약자 재안내 필요. 대상 건 목록은 관리자 예약 관리에서 방문일 미래 + 확정 필터로 확인.
  3. Pages 머지와 재배포 사이 잠깐 라벨(신)/캘린더(구) 불일치 가능 — 같은 시간대에 연달아 처리 권장.

---

## 2026-08-31 (5) — 월간 리포트: NPS 풋노트 신설 + 목적 도넛 위치 이동

- **운영자 지시 2건** (8월분 미리보기 리뷰): ① NPS 점수의 범위 표시 — 풋노트 방식 제안에 운영 세션 동의 (카드 안 병기는 협소, 풋노트가 비인지 수신자 교육 효과) ② 방문 목적별 분포를 사업부별 활용 현황 바로 아래로 이동 (방문 현황 블록끼리 묶고 큐레이션 블록은 뒤로).
- **NPS 풋노트 (HTML·텍스트 동일)**: KPI 카드 아래 1줄 — "* NPS(순추천지수): 추천 의향 0~10점 응답에서 추천(9~10점) 비율 − 비추천(0~6점) 비율. 범위는 −100~+100이며, 0보다 크면 추천 응답이 더 많다는 뜻" (문구는 실제 `classifySatisfaction` 계산 기준과 대조 확인). NPS 카드 라벨에 `*` 마커. **NPS 표시 월에만 조건부** — 구 척도 전용 월·무응답 월엔 풋노트·마커 모두 미표시.
- **섹션 순서 (신)**: ① Executive(+NPS 풋노트) ② 사업부별 활용 ③ **목적 도넛** ④ 핵심 인사이트 ⑤ 한마디 ⑥ 기사 ⑦ ROI — HTML 조립부·텍스트 빌더 두 벌 동일 적용 (텍스트판은 블록 이동, 복제 아님 검증).
- 검증: REPORT-LAYOUT 15항목 (HTML 조립 순서 4·풋노트 조건부·문구↔계산 기준 일치 / 텍스트 빌더 **실행** 검증 — 순서·중복 없음·집계 정상·구 척도 월 미표시) + 회귀(엔티티 19·ROI pin 23·슬롯 17) PASS. `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요 — 9/2(수) 08:30 자동 발송 전 필수**: 리포트 빌더 변경이라 미배포 시 8월분 발송본에 미반영. 재배포 후 [리포트 미리보기]로 순서·풋노트 확인 권장.

---

## 2026-09-01 (1) — 예약 D+7 버퍼 + 신청자/책임자 분리 (운영팀 요청 2건)

- **배경 (팀즈 운영 채널 → 운영자 전달)**: ① 이철호 책임 — 내일 슬롯이 갑자기 잡히면 대응 부담, 시스템은 1주일 버퍼·급한 건은 전화 수동 처리 ② 김현진 선임·서문수 선임 — B2B 영업 건은 관련부서를 한영본으로 넣는 관행상 책임자와 신청자가 달라 헷갈림, 신청자 칸 분리 요청.
- **① D+7 버퍼**: 예약 달력에서 오늘~D+6 선택 불가(minBookable) + 예약 안내 문구 추가 + **`booking` 서버 검증**(`date < KST 오늘+7일` → `booking_too_soon` 거부 — 공개 경로 우회 방지. no-cors라 클라이언트는 응답 미확인, 달력이 1차 게이트). `admin_booking_create`(백필)는 **의도적으로 제한 없음** — 전화 접수 긴급 방문의 등록 경로.
- **② 신청자/책임자 분리**: `HEADERS` 끝 `applicant`(신청자 "이름 직급") 추가 — **bookings 26컬럼**. `name`=방문 책임자로 의미 확정(관리자 표·캘린더 '책임자' 표기와 일치). 폼은 방문 책임자 이름·직급 + 「신청자가 책임자와 동일」 체크(기본 켜짐, 해제 시 신청자 칸 노출) — applicant는 항상 채워짐(동일 시 책임자 값 복사). 공란(분리 이전 행)=책임자와 동일 취급.
- **applicant 우선 소비처 (신청자=이메일 소유자=설문 작성자 정합)**: 확정·거절 메일 인사말 4곳(`applicant || name`) / 설문 초대 프리필 `name=` / 베스트 리뷰어 이메일 자동 매칭(applicant·name 모두 대조). 담당자 알림 메일(텍스트·HTML)·텔레그램은 책임자·신청자 다를 때만 신청자 줄 표기. 관리자: 상세 모달 신청자 행(다를 때만)·수정/백필 폼 신청자 입력(공란=동일)·CSV 신청자 컬럼·`admin_booking_edit` EDITABLE 등록.
- **privacy.html v1.3 개정**: §1 예약 신청 수집 항목에 방문 책임자 이름·직급 명시(신청자와 다른 경우) + 개정 이력 — 폼 동의 문구도 동기(수집 항목 변경 세트 규칙).
- 검증: APPLICANT-D7 26항목 (.gs 단위 — D+7 거부/접수·거부 시 무저장·26컬럼 위치·프리필 우선/폴백 / 배선 12 / 관리자 E2E — 상세 모달·수정 프리필·베스트 자동 매칭 / index E2E — 달력 D+7 전수·신청자 토글) + 회귀 197항목(10개 스위트) PASS. `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요**: `booking` 검증·HEADERS·메일 빌더 변경. 재배포 시점부터 bookings 시트에 `applicant` 헤더 자동 추가(getOrCreateHeaders 마이그레이션). **Pages 머지 전 재배포하면 잠깐 신규 예약의 applicant가 공란으로 저장되나 무해**(공란=동일 취급) — 순서 무관.

---

## 2026-09-01 (2) — 예약 폼 그룹 구분: 소속 귀속 혼선 해소 (운영자 피드백)

- **배경**: (1) 배포 직후 운영자 피드백 — 책임자·신청자 모두 사내 소속이 있는데 폼 상단 "소속 본부/부서"가 누구 것인지 불명확 (운영자 본인도 혼동).
- **의미 확정**: `division`/`department` = **방문을 주관하는 책임 부서의 소속** (B2B 대리 신청 시 한영본 기입 관행·사업부별 활용 통계·캘린더 표기와 정합 — 실사용과 일치하는 방향으로 확정). 설문 dept 프리필도 이 값 기준 — B2B 대리 케이스만 응답자가 수정하면 되는 수준.
- **폼 개편**: 「👤 방문 책임자」(소속 본부·부서 + 이름·직급) / 「✍️ 신청자」(동일 체크 + 조건부 이름·직급 + 이메일) **그룹 헤더·구분선으로 시각 분리**. 소속 부서 힌트를 "방문 책임자의 소속 부서명"으로, 신청자 그룹 부제에 "확정 메일·설문 수신" 명시.
- **문구 동기**: 폼 동의 문구·privacy.html §1을 "신청자·방문 책임자 이름·직급, 방문 부서 소속(본부·부서), 신청자 이메일"로 정리 (v1.3 문구 정제 — 항목 실질 동일, 버전 유지). data-schema division/department 의미 갱신.
- 검증: APPLICANT-D7 28항목 (그룹 헤더·귀속 힌트 2건 추가) PASS. HTML·문서만 변경 — **재배포 불필요**.
- **추가 반영 (같은 PR — 운영자 2차 피드백)**: 그룹 순서 교체 — **✍️ 신청자(폼 작성자) 상단**, 👤 방문 책임자(소속+동일 체크+조건부 이름·직급) 하단. 체크박스 방향 반전: 「방문 책임자가 신청자와 동일합니다」(기본 켜짐 — 해제 시 책임자 칸 노출). 페이로드 로직 동기(applicant=작성자 상시 입력, name=동일 시 복사·해제 시 별도 입력 — 시트 의미는 무변경). 검증 29항목 PASS.
- **추가 반영 2 (같은 PR — 운영자 3차 피드백)**: 「방문 책임자가 신청자와 동일합니다」 체크박스를 **책임자 그룹 헤더 바로 아래로 이동** (분기 결정이 그룹 첫 항목 — 소속 필드 아래 두면 체크가 소속에 걸리는 것처럼 오독). 순서: 헤더 → 동일 체크 → (해제 시) 책임자 이름·직급 → 소속 본부·부서(항상 필수).
- **추가 반영 3 (운영자 4차 피드백 — PR #96)**: 체크박스를 **질문형 + 기본 미체크**로 전환 — 「방문 책임자와 신청자가 동일합니까?」 질문 + 「예, 동일합니다」 체크(기본 꺼짐). 기본 체크 서술문은 읽지 않고 지나가는 패턴이라 능동 체크를 유도 (B2B 대리 신청 오입력 방지가 이 기능의 목적). **미체크 시 책임자 이름·직급 칸 기본 노출** — 미체크 상태의 의미 모호성 제거. 검증 토스트에 체크 안내 추가. 검증 31항목 PASS.

---

## 2026-09-01 (3) — 운영 기록: 운영팀 요청 반영 완료 (PR #94~97 전부 머지·반영 확인)

- **최종 상태**: ① D+7 예약 버퍼 + 신청자/책임자 분리(PR #94 — Apps Script 재배포 완료) ② 폼 그룹 분리·신청자 상단 배치(PR #95) ③ 동일 체크박스 그룹 상단 이동(PR #96) ④ 질문형+기본 미체크 전환(PR #97) — 운영자 피드백 4차 반복으로 폼 최종형 확정, 라이브 반영 확인.
- **폼 최종형**: ✍️ 신청자(이름·직급·이메일) → 👤 방문 책임자(「동일합니까?」 질문+「예, 동일합니다」 기본 미체크 → 미체크 시 책임자 이름·직급 칸 기본 노출 → 소속 본부·부서). CLAUDE.md의 폼 구성·name/applicant 제약 문구를 최종형으로 정리.
- **PR 머지 레이스 2회**(#96·#97 — 커밋 푸시 전 머지)는 리베이스 후 새 PR 분리로 처리 (기존 절차 준수, 커밋 유실 없음).
- 남은 운영 항목: 9월 첫 방문 건들에서 신청자≠책임자 케이스의 알림 메일·캘린더 표기 실측 확인 권장 (구조 검증은 테스트로 완료 — APPLICANT-D7 31항목).

---

## 2026-09-14 (1) — 운영 기록: "데모 데이터" 재발·해소 + 원탭 진단 페이지 신설 (코드 무변경)

- **증상**: 관리자 페이지가 데모 데이터 5건으로 표시 (2026-08-25에 이어 두 번째). 진단 결과 **판정 A — 접속 기기의 일시 네트워크 순단** (백엔드·배포본 정상, 공개 엔드포인트 JSON 정상 응답 확인). 새로고침으로 실데이터 복귀, 시트 무손상.
- **진단 과정 노트**: ① 9/1 이후 라이브 파일 변경 커밋 0건 확인(코드 회귀 배제 — 그 사이 main 유입은 이관 PR #93뿐, server/·docs만) ② 데모 폴백 발동 조건은 fetch 실패 또는 비JSON 응답 두 가지뿐 — **토큰 만료는 "세션 만료" 화면으로 빠지므로 데모의 원인이 아님** ③ 이번 클로드 세션 환경은 script.google.com 이그레스 차단이라 원격 curl 불가 — 사용자 기기 판별 절차로 전환.
- **원탭 진단 페이지 신설 (클로드 아티팩트 — 리포 외부)**: 「ThinQ Real 백엔드 진단」 — 공개 엔드포인트 2종(appliances·availability) 버튼 + 결과 판정 A(JSON=정상·순단→새로고침)/B(구글 오류=배포본 손상→새 버전 재배포)/C(차단 페이지=망 차단→타 망 교차 확인) 안내. 아이패드에서 URL 복사가 안 되는 상황 대응 — 운영자 클로드 계정의 아티팩트 갤러리(claude.ai/code/artifacts)에서 재사용 가능. **재발 시 표준 절차: 진단 페이지 버튼 → A/B/C 대조.**
- 참고: 같은 증상 2회 모두 원인이 순단(A) — 데모 데이터 표시는 데이터 소실이 아니라 연결 실패 폴백이며 캐시에 저장되지 않는다는 설계 확인 반복.

---

## 2026-09-14 (2) — 예약 버퍼 D+7 → D+2 단축 + 달력 아래 유선 안내 문구

- **운영자 지시**: 예약 가능 시작일을 오늘+2일로 단축 (예: 9/14 → 9/16부터 선택 가능. 구 D+7은 2026-09-01 도입 — 2주 운영 후 완화). 달력 아래에 안내 문구 추가 — 운영자 초안("방문 희망일이 2일 이내인 경우, ThinQ Real 담당자에게 유선으로 문의해 주시기 바랍니다")을 다듬어 "선택 불가 이유 + 유선 문의" 2문장으로: 「방문 희망일이 **2일 이내**인 경우 달력에서 선택할 수 없습니다. 급한 방문은 ThinQ Real 담당자에게 유선으로 문의해 주시기 바랍니다.」 (사이트에 유선 번호 미게재는 기존 의도 유지 — 번호 추가 없음).
- **변경 3곳**: index 달력 `minBookable` +7→+2 / 예약 상단 안내 "방문 희망일 2일 전까지"로 갱신 + 달력 카드 하단 신규 문구 / gs `handleNewBooking` 서버 검증 `7*86400000`→`2*86400000`. `admin_booking_create`(백필) 무제한은 그대로.
- 검증: APPLICANT-D7 32항목 (D+1 거부·D+2 접수·달력 당월 전수·문구 2종) PASS. `.gs` 문법 체크 PASS.
- **⚠ 재배포 필요**: `booking` 서버 검증 상수 변경 — 미배포 시 서버는 여전히 D+7 기준으로 거부하는데 달력은 D+2부터 열림 → **D+2~D+6 예약이 화면상 성공처럼 보이고 실제 저장 안 되는 불일치** (no-cors라 사용자에게 오류 미표시). Pages 머지와 재배포를 같은 시간대에 할 것 — **이번엔 순서가 중요** (재배포 먼저 권장: 서버가 먼저 D+2가 되면 과도기에도 무해).

---

## 2026-09-14 (3) — 달력 아래 안내 문구 2줄 분리 (운영자 가독성 피드백)

- PR #100 반영 확인 후 피드백: 안내 문구가 달력 박스 안에 한 문단으로 붙어 답답함 → **문장별 2줄 분리** + 달력 그리드와의 사이에 얇은 구분선·상단 여백 추가 (달력 카드 내 배치는 유지 — 시각적으로만 분리). 문구 내용 무변경. 검증 32항목 PASS. HTML만 — 재배포 불필요.

---

## 2026-09-14 (4) — 달력 안내 문구 시작점을 요일 「일」 열에 정렬 (운영자 피드백 3차)

- 피드백: 문구가 카드 왼쪽에 몰려 있음 — 원인: 달력 그리드는 좌우 24px 안쪽 여백인데 안내 블록은 여백 없이 카드 테두리에서 시작. **시작점을 요일 헤더 「일」 글자 위치로 정렬** — `padding-left: calc(18px + (100% - 72px)/14)` (그리드 좌 24px + 첫 칸 폭 절반 − 글자 절반 6px, 72=좌우 48+gap 24, 반응형 유지). 우측 24px·하단 18px 여백 동반.
- 검증: 픽셀 실측 — 데스크톱 오차 0.3px·모바일(390px) 0.2px. APPLICANT-D7 32항목 회귀 PASS. HTML만 — 재배포 불필요.

---

## 2026-09-14 (5) — 세션 마감 기록: PR #99~#102 전부 머지·반영 확인

- **오늘 완료 4건**: ① 데모 데이터 재발 → 순단(A) 확정·원탭 진단 페이지 절차 등재(#99) ② 예약 버퍼 D+7→D+2 단축 + 달력 아래 유선 안내(#100 — **재배포 완료**, 서버·달력 동기 확인) ③ 안내 문구 2줄 분리(#101) ④ 문구 시작점 요일 「일」 열 정렬 — 반응형 calc, 픽셀 실측 0.2~0.3px(#102). 모두 라이브 반영 확인.
- **현재 예약 정책 확정 상태**: D+2부터 선택 가능(달력+서버 이중), 2일 이내 긴급 방문 = 유선 접수 → 관리자 백필(제한 없음). 달력 아래 2줄 안내(「일」 열 정렬).
- **열린 항목 (변동 없음)**: 서면 동의서 법무 검토 대기 / 설문 원본 내보내기 멤버 피드백 대기(→ xlsx 탭본 판단) / Option B(전월 대비 증감) / CSE 403 / 신청자≠책임자 실건 발생 시 알림·캘린더 표기 실측 확인 / 10월 첫째 수요일(10/7) 9월분 리포트 자동 발송.

---

## 2026-09-14 (6) — 도어락 PIN 교체 (910910)

- **운영자 지시**: 도어락 비밀번호 변경 반영 — 확정 메일 빌더(텍스트·HTML) 2곳 + CLAUDE.md 기재값 교체 (구 PIN은 2026-07-20 교체분, 폐기). 민감 정보는 확정 메일에만 원칙 유지 — 메인 페이지·다른 파일에 잔존 0건 grep 확인.
- **운영 조치 3건**: ① **재배포 필수** — 미배포 시 새 확정 메일에 옛 PIN이 나감 ② **이미 확정 메일을 받은 미래 방문 건**은 옛 PIN이 적힌 메일을 보유 — 담당자가 해당 예약자에게 새 PIN 별도 안내 필요 (시스템 재발송 경로 없음 — 의도) ③ 이관 컨테이너(ST/QA/OP)의 env `DOORLOCK_PIN`도 교체 대상 (stage1-container-design §비밀값 — 이관 트랙/BE팀 처리).

---

## 2026-09-20 (1) — 개인정보 문서 법무 검토 확정본 반영: 처리방침 V3.0 + 수집·이용 동의서 V1.0 (국외 이전 조항 전면 삭제)

- **배경**: 사내 시스템 이관에 따라 개인정보 저장소가 Google Sheets → 사내 인프라(PostgreSQL)로 이동 — 운영자가 법무팀 검토를 거친 확정 문서 2건(Fin 2026-07-15, 시행 2026-09-01) 전달: ① 「개인정보 처리방침 V3.0」 ② 「개인정보 수집 및 이용 동의서 V1.0」. 지시: Google LLC·국외 이전 등 구 내용을 전 파일에서 검토본 기준으로 교체.
- **privacy.html — v1.3 → V3.0 전면 개정**: 법무 검토본 14개 조항 체계로 교체 (목차 포함). 핵심 변경 — §1 수집 항목 「방문자 명단(소속·이름·직급) + 방문 정보(목적·주제·일자·회차)」 / §3 보유 기간 「**방문예약일로부터 3년** + 분쟁 시 해결 시까지」(구 "방문일로부터 3년") / §4 제3자 제공 없음 / §5 위탁 없음 / **§7 「회사는 이용자의 정보를 국외로 이전하지 않습니다」** (구 §4 Google LLC 국외 이전 5요소 표 삭제) / §9 사내 SSO 접근 통제 명시 / §12 보호책임자(LG전자 정보보호담당)·운영책임자(AI홈솔루션엔지니어링팀 팀장 김재훈)·보호담당·운영담당 / §13 권익침해 구제 기관 4곳. 버전 이력에 v1.0~1.3 보존 + 3.0 추가 (버전 체계를 법무 검토본 기준으로 전환 — v2.x 없음이 정상).
- **ThinQ_Real_Visit_Consent.html — 서식 v0.9 → V1.0 전면 교체**: 제목 「개인정보 수집 및 이용 동의서」, 단일 동의 구조(목적/항목/기간 내역 표 + 거부권 고지 + 「동의하십니까?」 질문). **구 2항(국외 이전)·3항(현장 대화 녹음) 삭제** — 확정본에 해당 조항 없음. 서명 명부는 현장 운영용으로 유지하되 검토본의 동의함/동의하지 않음 선택을 「동의 여부(○ 표시)」 컬럼으로 서식화 + 서명 컬럼 분리. 관리자 [🖨 동의서] 프리필 매핑(tds[1] 성명·tds[2] 소속·직급) 무변경 호환.
- **⚠ 스펙 대비 변경 (서식화 판단)**: 검토본의 개별 체크박스(□ 동의함 □ 동의하지 않음)를 단체 방문 명부 형식(행별 ○ 표시 + 서명)으로 옮김 — 인원별 능동 선택은 보존. 검토본 원문에 없는 서명 명부·방문일/회사/담당자 기입란은 현장 운영 요구(2026-08-31 프리필 흐름)로 유지.
- **index.html 예약 폼**: 국외 이전 동의 체크박스(fPrivacyTransfer) **삭제** → 필수 동의 3종 → 2종(수집·이용/파손·분실). 수집·이용 동의 문구를 동의서 V1.0 내역(목적 전체 열거 + 방문자 명단·방문 정보 + 방문예약일로부터 3년)으로 교체, 거부권 문구도 검토본 문장으로 교체. payload `privacyConsent:'Y'`는 유지 (의미가 "수집·이용 동의"로 축소 — 구 행의 'Y'는 국외 이전 포함 의미, .gs HEADERS 주석·data-schema.md에 기재).
- **CLAUDE.md**: 동기화 지점 개정(보유 기간 "방문예약일로부터 3년" 3곳 동기 / 국외 이전 조항 재도입 금지 — 법무 재검토 전제), 필수 동의 2종, 파일 구조 버전 표기, 리마인드 「서면 동의서 법무 확정」 완료 처리.
- **미해결 확인 포인트 (리마인드 등재)**: ① 검토본 수집 항목에 **신청자 이메일·임직원 인증 이메일·방문 후기 설문(성명·소속) 미포함** — 현행 시스템은 계속 수집 중이므로 법무 항목 추가 확인 또는 SSO 전환 시 수집 중단 시점 확인 필요 ② **머지=즉시 라이브 게시** — §7(국외 이전 없음)은 저장소 사내 이전을 전제하므로, Google Sheets 저장이 지속되는 현시점 게시 정합은 운영자·법무 판단 사안 ③ FieldVoice 현장 녹음 동의는 확정본 범위 밖 — 실운영 시 별도 법무 확정 필요.
- HTML·문서 중심 — `.gs`는 주석 1곳만 변경(**재배포 불필요**).

---

## 2026-09-20 (2) — resetForm() 신청자/책임자 분리 필드 초기화 누락 수정

- **증상**: 예약 완료 후 [새 예약 신청] 버튼의 `resetForm()`이 2026-09-01 신청자/책임자 분리(PR #94~97) 이후 추가된 필드를 초기화하지 않음 — 신청자 이름(`fAppName`)·직급(`fAppRank`) 값과 「예, 동일합니다」 체크(`fSameLeader`)가 잔존. 이메일은 지워지는데 신청자 이름·직급만 남아 비일관, fSameLeader가 체크된 채 남으면 책임자 칸(leaderRow) 표시 상태가 새 폼 기본값(미체크·노출)과 어긋남.
- **수정**: 초기화 배열에 `fAppName`·`fAppRank` 추가 + `fSameLeader.checked = false` 후 `toggleLeader()` 호출로 leaderRow 노출 복원. leaderRow는 정적 마크업이라 `renderFormByPurpose()` 순서 무관. 리셋 정책은 전체 초기화(최초 진입과 동일 상태) — 연속 예약 시 신청자 정보 유지 편의는 채택하지 않음(필요 시 운영자 판단으로 후속).
- 검증: 민감 단가 grep 0건(규칙 문서 인용 제외). HTML만 — 재배포 불필요.

---

## 2026-09-20 (3) — 서면 동의서 V1.1: 현장 녹음 동의 조항 재추가 (운영자 지시)

- **운영자 지시**: PR #125(법무 확정본 V1.0 반영 — 녹음 조항 미포함) 머지·반영 확인 후, "실제 공간에서 녹음을 진행하기 때문에" 기존 녹음 동의는 서면 동의서에 그대로 유지하라고 지시.
- **반영**: `ThinQ_Real_Visit_Consent.html` 서식 V1.0 → **V1.1** — ① 1항 「개인정보 수집 및 이용 동의」(법무 검토본 원문 무변경) 뒤에 **2항 「현장 대화 녹음 및 인사이트 활용 동의」 재추가** (구 v0.9 3항 **최종 문안 그대로**: 가명처리 요약 한정·원본/전문 외부 미공유·요약 후 지체 없이 파기 + 거부권·불이익 없음 안내. 2026-08-31 운영자 지시였던 "(선택)" 워딩 삭제 상태 유지) ② 서명 명부에 「2항 녹음 동의(동의 시 ○)」 열 추가 (6컬럼: 번호/성명/소속·직급/1항 동의 여부/2항 녹음 동의/서명 — 컬럼 폭 재배분으로 A4 유지) ③ 동의 질문 박스에 1항·2항 기입 안내 병기. 관리자 [🖨 동의서] 프리필 매핑(tds[1]·tds[2])은 컬럼 추가와 무관 — 무변경 호환.
- **⚠ 스펙 대비 변경**: 2항(녹음)은 **법무 검토 확정본(Fin 2026-07-15) 범위 밖** — 운영자 지시로 유지. 법무 확인 포인트로 존치: 녹음 조항 자체의 법무 검토, FieldVoice `consent` 기록 방식과 문안 정합, **처리방침 V3.0에 녹음 관련 수집·이용 기재 없음**(동의서에만 존재하는 상태) — CLAUDE.md 리마인드 「개인정보 문서 V3.0 게시 후속 확인」 ③으로 등재.
- 검증: 정적+런타임 19항목 PASS (1항 법무 문안 무변경·2항 문안 v0.9 일치·"(선택)" 워딩 없음 유지·명부 10행×6칸·프리필 호환·인쇄 A4 1페이지 실측 1002px). HTML만 — 재배포 불필요.

---

## 2026-09-20 (4) — 서면 동의서 명부 1항 동의란 단순화 (운영자 피드백)

- **피드백**: 명부의 1항 동의란에 「동의함 · 동의하지 않음」이 한 칸에 병기되어 복잡해 보임 — 2항(녹음)과 같은 「동의 시 ○」 스타일로 통일 요청 (실사용 스크린샷 기반, 프리필 동작은 정상 확인됨).
- **반영**: `ThinQ_Real_Visit_Consent.html` 명부 — 1항 열 헤더 「1항 동의 여부(하나에 ○ 표시)」 → **「1항 동의(동의 시 ○)」**, 각 행의 「동의함 · 동의하지 않음」 프리텍스트 삭제(빈 칸 — 2항과 동일), 열 폭 132→76px(2항과 동일), 동의 질문 박스 안내문 한 문장으로 단순화(「동의하시는 항의 란에 ○ 표시 후, 서명란에 서명해 주시기 바랍니다.」). 미사용된 `choice-cell` CSS 제거. **거부권 보장은 1항 본문 ※ 고지가 담당** — 명부 상 미동의 = 무표시. 조항 문안 무변경(서식 V1.1 유지).
- 프리필 매핑(tds[1]·tds[2]) 무변경 호환. HTML만 — 재배포 불필요.
- **추가 반영(같은 PR #128)**: 관리자 [🖨 동의서] 프리필에 **안내 담당자 = 예약의 방문 책임자(`name`, "이름 직급")** 명기 (운영자 지시 — 현장 안내를 책임자가 수행하는 운영 흐름). 동의서 쪽 프리필 스크립트는 기존에 `docent` 키를 이미 처리하고 있어 관리자 페이로드에 값만 추가. 공란(구 백필 행)이면 서식에서 수기 기입.

---

## 2026-09-21 (1) — 설문 탭 자동 최신화 + 9/20 이정민 응답 미표시 소동 규명

- **증상 신고**: 텔레그램으로 설문 접수 알림(방문일 2026-09-09 · 2회차 · 이정민)이 9/20에 왔는데 관리자 설문·대장 탭에서 안 보임.
- **규명**: 시트 원본 직접 조회(스크립트 소유 계정 Drive 연결)로 `survey_responses` 행 정상 존재 확인 (response_id 1789870770532, sales 트랙 — 대장 파생 없음은 확정 설계). 원인은 **관리자 설문 탭의 인메모리 캐시**: `loadSurveyData`가 캐시 있으면 재조회를 생략해, 열어 둔 페이지에서는 새 응답이 수동 새로고침 전까지 미표시. 운영자가 새로고침으로 확인 완료 — 데이터 유실 아님.
- **개선 (운영자 지시)**: 설문 탭도 예약 탭과 동일한 **stale-while-revalidate**로 전환 — 탭 재진입 시 캐시 즉시 렌더 + 백그라운드 조용한 재조회(`loadSurveyData` 자동 경로가 내부적으로 force+quiet 재호출, `svAutoRefreshing` 가드로 연타 중복 조회 방지). 수동 ↺ 새로고침(토스트)·변이 후 재동기화(`loadSurveyData(true, true)`) 경로는 무변경. localStorage 캐시는 도입하지 않음(개인정보인 설문 원문을 브라우저에 남기지 않음 — 예약 캐시와 달리 의도적 미사용).
- 검증: Playwright 8항목 PASS — 최초 진입 1회 조회, 재진입 시 백그라운드 재조회로 **새 응답(이정민 케이스 재현)이 새로고침 없이 자동 표시**, 연타 중복 방지, 수동 새로고침 경로 유지, JS 오류 0건.
- 관리자 HTML만 — **재배포 불필요**.
