...
- **① 가치 항목 한글 명칭 개편 (전 지점)**: R&D 효율화→**R&D 기여 가치** / 영업 지원(인프라)→**영업 기여 가치** / 영업 지원(기여이익)→**기여 영업 이익** / PR 가치→**홍보 노출 가치** / 품질 가치→**품질 개선 가치**. 반영 3곳 — ROI 툴(요약 스트립·패널 h2·팝업 제목·차트 라벨·본문 21건), admin 동작 원리 카드(6건), gs `ROI_VALUE_LABELS`(라벨 갱신 + **vQuality 항목 추가** #1b6ca8 — collectOutputs 키 매칭 규칙 유지). 심볼(V_R&D 등)은 수식 표기라 불변. CLAUDE.md 동기화 지점에 명칭 3곳 동기 규칙 추가. `ROI_BUILD` '20260805c'.
