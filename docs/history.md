...
## 2026-09-17 — 사내 이관 외주 견적 문의 (오큐브) 회신 구성 + 이관 난이도 자체 진단

- **배경**: 팀 회의에서 비개발자 운영자의 사내 이관 진행 속도 이슈 논의 → 팀장 지시로 「잔여 이관」과 「향후 유지보수」를 나눠 M/D·기간·비용을 외주(협력사 오큐브, 창원 신축기숙사 스마트솔루션 건 협업사)에 문의. 9/17 협력사가 견적 전 6개 문의를 회신 → 운영 세션이 리포 이관 문서(`docs/migration/`)와 시트 실데이터 집계를 근거로 회신 문안 구성.
- **협력사 문의 6건 ↔ 회신 요지** (문안 전문은 운영자 메일로 발송 — 리포 미보관):
  1. 현행 아키텍처 → 현행(GitHub Pages HTML 6종 + Apps Script 5,146줄 GET 19/POST 30 + Sheets 13탭 + Gmail·Telegram·Calendar·Serper) / 이관 목표(Node 22+Express 단일 컨테이너, Gitea push 자동배포, TCN K8s ST/QA/OP, PostgreSQL+Valkey, Vault, ops-gateway SSO + `/pub` 예외 5종, Teams 웹훅, 사내 SMTP 대기, `thinqreal.lge.com` CSR 등록 완료·라우팅 대기). 진척: **0.10.0이 3환경 가동, ST PostgreSQL 연결 확인**. 문서는 퍼블릭 리포 `docs/migration/` 열람 안내 (사내 식별자는 미팅에서 구두).
  2. QE/QA → 별도 계획 없음(자체 회귀 + ST 검증표 + QA 리허설 예정). **「전환 검증(기능 회귀 + 데이터 이행 정합성)」을 별도 견적 항목으로 분리 요청**.
  3. 모니터링/Alert → 카카오톡·PagerDuty 아님. **MS Teams 채널 알림** — ① 장애(healthz 실패·5xx·일일 배치 실패) ② 업무 이벤트(텔레그램 대체). 로그·메트릭은 TCN LENS.
  4. 인프라 구축 주체 → **구축 불필요** (BE팀이 클러스터·CI/CD·게이트웨이·3환경 구축 완료). 남은 것은 OP 자원 신청(RDS·ElastiCache JIRA / Next SPoC / Vault)이며 **신청·결재 주체는 LG전자 담당자여야 함** — 업체 역할은 기재값 정리·기술 검토·발급 후 연결 설정(env/Vault)·배포 검증.
  5. 이관 제약 시 기능 수정 → 가능(현행·컨테이너 코드 모두 운영자가 작성·관리). 견적은 (a) 자체 수정 / (b) 업체 수정 두 가정 분리 요청, 유지보수 계약 후에는 업체 주체 전제.
  6. 데이터량 → **Sheets 1파일 약 93KB, 13탭, 총 약 480행** (bookings 94 / health_checks 206 / visitor_responses 67 / slot_blocks 35 / survey_responses 24 / monthly_insights 18 / monthly_articles 15 / export_log 13 / performance_ledger 5 / roi_snapshots 2 / iot_issue_log·best_reviewers·voc_reports 0 — 2026-09-17 집계, Drive 커넥터로 행수만 산출·내용 미기록). 정적 자산(images 21개 6.9MB)은 컨테이너 이미지 포함이라 이행 대상 아님. 이행 방식은 과제 D 설계(스냅샷 JSON → `admin_import` 업로드 → manifest 대조) 완료 상태 → **공수는 데이터량이 아니라 전환 절차에서 발생**.
  - **견적 요청 구도 제안**: A. 잔여 이관 전체 위탁(사내 절차 지원+연결+이행+검증+인수인계) / B. 이관은 자체 완료 후 유지보수만(연간 운영 대응, SLA 옵션별). 온라인 미팅 제안 포함.
- **이관 난이도 자체 진단 (운영 세션 판단)**:
  - 끝난 것: 코드 이관 전부(API 49종·PG 어댑터·인앱 스케줄러·`/pub`). 남은 것: **사내 자원 신청 4건 + BE팀 회신 대기 3건(SSO 예외 반영·lge.com 라우팅·SMTP) + 과제 D 이행 1회** — 개발이 아니라 절차.
  - "만들 때보다 힘들다"는 체감의 원인은 구조: 상대 조직 4곳(BE·인프라·DB·CSR) / 가이드가 신청서 항목 수준 / 사내→외부 정보 채널이 타이핑뿐 / 대기 시간(SMTP 8월 말~). 역량 문제 아님 — CSR 등록·`/pub` 분리안 관철·Gitea 이력 재작성 무손실 대응·사내 Claude 문맥 팩 등 이미 수행한 기록이 근거.
  - **실제 리스크 3**: ① 12월 완료 일정은 DB팀·인프라팀·BE팀 처리 속도에 의존 — 외주로도 신청 주체가 바뀌지 않아 병목 미해소 ② 코드·절차·맥락의 1인 의존(bus factor) ③ 전환 후 장애 시 컨테이너 코드를 고칠 사람 부재 → 유지보수 외주의 정당한 근거.
  - **권고**: 잔여 이관은 자체(운영자+BE팀+사내 Claude) 유지, 외주 가치는 **전환 검증(QA)·인수인계 검수·연간 유지보수**에. 견적 A가 저렴해도 "사내 절차는 업체가 대신 못 함"을 팀장 보고에 명시. 용어·판단 보조가 목적이면 프로젝트 위탁보다 시간제 기술 자문이 적합할 수 있음.
- 코드·문서 변경 없음 (이 기록 + CLAUDE.md 리마인드 1줄만). 견적 회신 수신 후 팀장 결정 → 후속 기록 예정.
