...
## 2026-09-16 (1) — 운영 기록: AX 비용 효과 산정 과제 수행 (코드 무변경)

- 회사 요청(AX 비용 효과 산정 — 외주 대체 비용 + 산정 로직·기준 표)에 따라 현행 시스템 전체의 외주 대체 비용을 산정. 수량 근거는 리포 실측(화면·API·시트 탭·메일 템플릿·LOC), 단가는 ROI 툴과 동일한 KOSA 공표 통계 보수 적용, 투입공수 방식 + 기능점수 교차 검증.
- **산출물(보고서·수치)은 사내 보고물이라 리포 미포함** — 커밋 금지 규칙(보고 자료) 준수, 아티팩트·파일로 운영자에게 전달. 후속(제출 양식 맞춤·실투입 빈칸 확정·단가 최신본 검증)은 사내 클로드 별도 세션에서 진행 — 인수인계 브리핑(첫 프롬프트 일체형) 전달됨. 이관 트랙과 별개 과제로 선언(브리핑 상단 명시).
