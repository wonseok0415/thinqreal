...
import { deleteLegacyVisitor } from '../jobs/edgeSync.js';
...
export async function handleVisitorDelete(store, data) {
  ...
  // 하이브리드 에지(§8-10): 원본은 현행 Apps Script 시트 — 거기서도 지워야 다음 pull에서 부활하지 않는다
  const remote = await deleteLegacyVisitor(data.id);
  return { ok: true, ...(remote.skipped ? {} : { legacyDeleted: !!remote.ok }) };
}
