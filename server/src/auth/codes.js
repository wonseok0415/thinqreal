...
// 저장은 kvcache(공유 캐시) 경유 — KVSTORE_ADDR 있으면 Valkey(멀티 레플리카 공유), 없으면 메모리.
import crypto from 'node:crypto';
import { kvGet, kvPut, kvDel } from '../lib/kvcache.js';
import {
...
export async function issueCode(email, kind) {
  const coolKey = `${kind}_cool_${email}`;
  if (await kvGet(coolKey)) return null;
  // 6자리 코드 — 앞자리 0 허용, CSPRNG 사용
  let code = '';
  for (let i = 0; i < 6; i++) code += crypto.randomInt(10);
  await kvPut(`${kind}_code_${email}`, code, AUTH_CODE_TTL_SEC);
  await kvPut(coolKey, '1', AUTH_COOLDOWN_SEC);
  return code;
}
