...
function handleNewBooking(data) {
  ...
  const minDate = Utilities.formatDate(new Date(Date.now() + 2 * 86400000), 'Asia/Seoul', 'yyyy-MM-dd');
...
function buildConfirmHtml(data) {
  ...
  const rows =
    ...
    infoRow('📶', '무선 인터넷',
      ...
      '</table>') +
    infoRow('🔐', '도어락 비밀번호',
      '<div style="font-family:Consolas,Menlo,monospace;font-size:15px;color:#1d1d1f;letter-spacing:0.04em;">910910</div>') +
    infoRow('🅿', '주차',
